...
INSERT
IGNORE INTO b_cache_tag (SITE_ID, CACHE_SALT, RELATIVE_PATH, TAG)
				VALUES
			('s1', '/eda', '/s1/bitrix/menu/06f', 'bitrix:menu');
SELECT *
FROM `b_forum_user` LIMIT 0;
UPDATE b_forum_user
SET LAST_VISIT=now()
WHERE USER_ID = 1;
UPDATE b_forum_stat
SET `USER_ID`    = 1,
    ...
    `FORUM_ID`   = 0,
    `TOPIC_ID`   = 0
WHERE PHPSESSID = 'o50tf3qfh12hfbbfcpis0npeb2';
...
SELECT
				COUNT(ST.ID) as CNT
FROM b_sticker ST
WHERE ((ST.PERSONAL = 'Y' AND ST.CREATED_BY = 1) OR ST.PERSONAL = 'N')
  AND ST.CLOSED = 'N'
  AND ST.DELETED = 'N'
  AND ST.SITE_ID = 's1'
...
FROM b_forum_user FU
         LEFT JOIN b_user U ON (FU.USER_ID = U.ID)
WHERE 1 = 1;
SELECT FU.ID,
       U.ID                                                                                                                                                                                             as USER_ID,
       FU.SHOW_NAME,
...
WHERE PHPSESSID = 'o50tf3qfh12hfbbfcpis0npeb2';
SELECT U.*,
       DATE_FORMAT(U.TIMESTAMP_X, '%d.%m.%Y %H:%i:%s')                           TIMESTAMP_X,
...
WHERE 1 = 1
  AND ((VQ.VOTE_ID = 1))
  AND ((VA.ACTIVE = 'Y'))
  AND ((VA.QUESTION_ID IN (1)))
ORDER BY VA.C_SORT ASC;
SELECT VEQ.QUESTION_ID,
       VEA.ANSWER_ID,
       COUNT(VEA.ID)                                   as COUNTER,
...
WHERE 1 = 1
  AND ((VE.VOTE_ID = 1))
  AND ((VE.VALID = 'Y'))
GROUP BY VEQ.QUESTION_ID, VEA.ANSWER_ID
...
WHERE (ID = '1')
  AND (AUTH_USER_ID = '1');
INSERT INTO b_vote_event(`VOTE_ID`, `VOTE_USER_ID`, `DATE_VOTE`, `STAT_SESSION_ID`, `IP`, `VALID`)
VALUES (1, 1, now(), 0, '127.0.0.1', 'Y');
INSERT INTO b_vote_event_question(`EVENT_ID`, `QUESTION_ID`)
VALUES (2, 1);
INSERT INTO b_vote_event_answer(`ANSWER_ID`, `EVENT_QUESTION_ID`)
VALUES (1, 2);
UPDATE b_vote
SET `COUNTER` = COUNTER + 1
WHERE ID = '1';
UPDATE b_vote_question
SET `COUNTER` = COUNTER + 1
WHERE ID in (1);
UPDATE b_vote_answer
SET `COUNTER` = COUNTER + 1
WHERE ID in (1);
UPDATE b_vote_user
SET `DATE_LAST` = now(),
