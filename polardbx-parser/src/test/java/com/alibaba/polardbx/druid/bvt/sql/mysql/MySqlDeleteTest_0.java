...
//        System.out.println("orderBy : " + visitor.getOrderByColumns());
    ...
    public void test_1() throws Exception {
        String sql = "DELETE /*TDDL: cost()*/ a1, a2 FROM t1 AS a1 INNER JOIN t2 AS a2 WHERE a1.id=a2.id;";

        MySqlStatementParser parser = new MySqlStatementParser(sql, SQLParserFeature.TDDLHint);
        List<SQLStatement> statementList = parser.parseStatementList();
        MySqlDeleteStatement stmt = (MySqlDeleteStatement)statementList.get(0);
//        print(statementList);

        System.out.println(stmt);

        Assert.assertEquals(1, statementList.size());
        Assert.assertEquals(1, stmt.getHintsSize());
        Assert.assertEquals("TDDL: cost()", stmt.getHints().get(0).getText());
