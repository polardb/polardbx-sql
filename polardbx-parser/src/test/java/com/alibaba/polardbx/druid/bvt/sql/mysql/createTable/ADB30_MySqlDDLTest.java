...
//        parseTrue("alter table ddlDb.tbl rename tbl2", "RENAME TABLE ddlDb.tbl TO tbl2");
        ...
        parseTrue("alter table ddlDb.addColumn add column c1 double(2,4)", "ALTER TABLE ddlDb.addColumn\n" +
            "\tADD COLUMN c1 double(2, 4)");
        parseTrue("alter table ddlDb.addColumn add column c2 int(3) NOT NULL DEFAULT 123",
            "ALTER TABLE ddlDb.addColumn\n" +
                "\tADD COLUMN c2 int(3) NOT NULL DEFAULT 123");
        parseTrue(
            "alter table ddlDb.addColumn add column c1 double(4, 5) NOT NULL DEFAULT 1.23 AUTO_INCREMENT ENCODE='AUTO' COMPRESSION='SNAPPY'",
            "ALTER TABLE ddlDb.addColumn\n" +
                "\tADD COLUMN c1 double(4, 5) NOT NULL DEFAULT 1.23 AUTO_INCREMENT ENCODE='AUTO' COMPRESSION='SNAPPY'");

        parseTrue(
            "alter table ddlDb.addColumn add column c1 multivalue NOT NULL AUTO_INCREMENT ENCODE='AUTO' COMPRESSION='SNAPPY' delimiter_tokenizer ': ,' value_type 'varchar int'"
            , "ALTER TABLE ddlDb.addColumn\n" +
    ...
    public void test_truncate() {
        ...
        parseTrue("TRUNCATE table ddlDb.truncateTable partition all",
            "TRUNCATE TABLE ddlDb.truncateTable PARTITION ALL");
        parseTrue("TRUNCATE table ddlDb.truncateTable partition 1,4,5",
            "TRUNCATE TABLE ddlDb.truncateTable PARTITION 1, 4, 5");
    }
    ...
    public void test_restore_2() throws Exception {
        SQLStatement parse = parse(
            "restore data from '[{\"dbname\":\"backup\",\"url\":\"testBackup/BACKUP_20_20180614172606\",\"upload_id\":20,\"check_type\":\"MD5\",\"checksum\":\"\",\"log_pos\":0,\"file_size\":83717}]','oss://test:test@oss-cn-hangzhou-zmf.aliyuncs.com/039696/testBackup/'");
        System.out.println(parse);
    }

    public void test_build() throws Exception {
        parseTrue("build table a.b ", "BUILD TABLE a.b FORCE = false");
        parseTrue("build table a.b version = 1000 with split",
            "BUILD TABLE a.b VERSION = 1000 WITH SPLIT FORCE = false");
        parseTrue("build table a.b version = 1000 ", "BUILD TABLE a.b VERSION = 1000 FORCE = false");
        parseTrue("build table a.b with split ", "BUILD TABLE a.b WITH SPLIT FORCE = false");
    }
    ...
    public void test_database() throws Exception {
        parseTrue(
            "create database if not exists test__1 options(shards=1 shard_id=1 replication=1 storage_dependency='INTERNAL')",
            "CREATE DATABASE IF NOT EXISTS test__1 OPTIONS (replication=1 shards=1 storage_dependency='INTERNAL' shard_id=1 )");
        parseTrue(
            "create schema if not exists test__1 options(shards=1 shard_id=1 replication=1 storage_dependency='INTERNAL')",
            "CREATE DATABASE IF NOT EXISTS test__1 OPTIONS (replication=1 shards=1 storage_dependency='INTERNAL' shard_id=1 )");
        parseTrue(
            "create schema if not exists test__1  shards=1 shard_id=1 replication=1 storage_dependency='INTERNAL'",
            "CREATE DATABASE IF NOT EXISTS test__1 OPTIONS (replication=1 shards=1 storage_dependency='INTERNAL' shard_id=1 )");
    }
    ...
    public void test_procedure() throws Exception {
        parseTrue(
            "CREATE procedure pdb.p2() COMMENT 'COMMENT...' LANGUAGE SQL SQL SECURITY DEFINER ( select 1 ) union all (select 2)",
...
//    }
    ...
    public void test_import_table() throws Exception {
        ...
        parseTrue("import table tablename version = 123 build='Y'",
            "IMPORT TABLE tablename VERSIOIN = 123 BUILD = 'Y'");
    }
