    ...
    public void test_one() throws Exception {
...

        {
            ...
            Assert.assertEquals("CREATE TABLE `app_customer_license` ("
                    ...
                    + "\n\t`customer_info` bigint(20) NULL DEFAULT NULL,"
                    + "\n\tPRIMARY KEY (`id`),"
                    + "\n\tCONSTRAINT FOREIGN KEY (`customer_info`) REFERENCES `app_customer_info` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,"
                    + "\n\tINDEX `fk_app_customer_info_id` USING BTREE(`customer_info`),"
                    + "\n\tUNIQUE `idx_app_customer_license_deviceId` USING BTREE (`device_id`)"
                    + "\n) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8 DEFAULT COLLATE = utf8_general_ci AUTO_INCREMENT = 1 ROW_FORMAT = DYNAMIC",
                output);
        }
