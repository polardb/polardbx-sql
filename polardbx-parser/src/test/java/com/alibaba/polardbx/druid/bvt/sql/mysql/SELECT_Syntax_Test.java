...
public class SELECT_Syntax_Test extends TestCase {
    ...
    public void testFlashback() throws Exception {
        String sql = "SELECT s1 FROM t1 AS OF TIMESTAMP '2021-03-02 11:45:14';";

        SQLStatementParser parser = new MySqlStatementParser(sql);
        List<SQLStatement> stmtList = parser.parseStatementList();
        SQLStatement stmt = stmtList.get(0);

        Assert.assertEquals("SELECT s1\nFROM t1\nAS OF TIMESTAMP '2021-03-02 11:45:14';", SQLUtils.toMySqlString(stmt));
        Assert.assertEquals("select s1\nfrom t1\nas of timestamp '2021-03-02 11:45:14';", SQLUtils.toMySqlString(stmt, SQLUtils.DEFAULT_LCASE_FORMAT_OPTION));
    }
