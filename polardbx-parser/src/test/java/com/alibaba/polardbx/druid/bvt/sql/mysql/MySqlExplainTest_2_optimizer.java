...
public class MySqlExplainTest_2_optimizer extends MysqlTest {

    public void test_0() throws Exception {
        String sql = "explain optimizer select 1";

        MySqlStatementParser parser = new MySqlStatementParser(sql);
        List<SQLStatement> statementList = parser.parseStatementList();
        SQLStatement stmt = statementList.get(0);

        assertEquals("EXPLAIN OPTIMIZER SELECT 1", stmt.toString());
        assertEquals("explain optimizer select 1", stmt.toString().toLowerCase());
//        print(statementList);
