...
public class DALParserTest extends TestCase {
    ...
    public void test_show_index_bad() throws Exception {
        String sql = "SHOW index";
        MySqlStatementParser parser = new MySqlStatementParser(sql);
        try {
            parser.parseStatementList();
            Assert.fail("Should fail.");
        } catch (Exception e) {
            Assert.assertTrue(e.getMessage().contains("syntax error, expect FROM or IN, actual EOF"));
        }
    }

    public void test_show_index_bad_1() throws Exception {
        String sql = "SHOW indexes";
        MySqlStatementParser parser = new MySqlStatementParser(sql);
        try {
            parser.parseStatementList();
            Assert.fail("Should fail.");
    ...
    public void test_show_profile_1() throws Exception {
        String sql =
            "SHOW profile all,block io,context switches,cpu,ipc,memory, page faults,source,swaps for query 2 limit 1 offset 2";
        MySqlStatementParser parser = new MySqlStatementParser(sql);
        MySqlShowProfileStatement show = (MySqlShowProfileStatement) parser.parseStatementList().get(0);
        parser.match(Token.EOF);
        String output = SQLUtils.toMySqlString(show);
    ...
    public void test_show_trace_04() {
        String sql = "SHOW TRACE (ROWS+1, ID*2) WHERE 1=1 limit 1";
        MySqlStatementParser parser = new MySqlStatementParser(sql);
        MySqlShowTraceStatement show = (MySqlShowTraceStatement) parser.parseStatementList().get(0);
        parser.match(Token.EOF);
        String output = SQLUtils.toMySqlString(show);
        Assert.assertEquals("SHOW TRACE(ROWS + 1, ID * 2) WHERE 1 = 1 LIMIT 1", output);
    }
    ...
    public void test_show_table_info() throws Exception {
        String sql = "SHOW TABLE INFO T1 WHERE 1=1 limit 1";
        MySqlStatementParser parser = new MySqlStatementParser(sql);
        MySqlShowTableInfoStatement show = (MySqlShowTableInfoStatement) parser.parseStatementList().get(0);
        parser.match(Token.EOF);
        String output = SQLUtils.toMySqlString(show);
        Assert.assertEquals("SHOW TABLE INFO FROM T1 WHERE 1 = 1 LIMIT 1", output);
    }
