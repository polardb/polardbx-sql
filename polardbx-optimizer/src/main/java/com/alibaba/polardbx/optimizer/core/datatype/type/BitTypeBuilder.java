...
public class BitTypeBuilder extends AbstractBasicSqlTypeBuilder {
    public static BitTypeBuilder INSTANCE = new BitTypeBuilder();

    private BitTypeBuilder() {
    }
    ...
    SqlTypeName getTypeName(SQLDataType dataType) {
        if (getPrecision(dataType) == 1) {
            return SqlTypeName.BIT;
        } else {
            return SqlTypeName.BIG_BIT;
        }
    }
}
