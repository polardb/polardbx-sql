...
import com.alibaba.polardbx.optimizer.state.NullableFloatGroupState;

public class Float2FloatMax extends AbstractAggregator {

    protected NullableFloatGroupState groupState;

    public Float2FloatMax(int index, DataType inputType, DataType outputType, int filterArg) {
        super(new int[] {index}, false, new DataType[] {inputType}, outputType, filterArg);
    }

    @Override
    public void open(int capacity) {
        groupState = new NullableFloatGroupState(capacity);
    }
    ...
    public void accumulate(int groupId, Chunk chunk, int position) {
        ...
        if (groupState.isNull(groupId)) {
            ...
            float afterValue = Math.max(value, beforeValue);
    ...
    public void writeResultTo(int groupId, BlockBuilder bb) {
        if (groupState.isNull(groupId)) {
            ...
            bb.writeFloat(groupState.get(groupId));
        }
    }
