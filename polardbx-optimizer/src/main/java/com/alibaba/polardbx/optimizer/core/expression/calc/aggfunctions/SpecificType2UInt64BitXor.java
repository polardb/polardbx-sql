...
public abstract class SpecificType2UInt64BitXor extends AbstractAggregator {
    private NullableUInt64GroupState groupState;

    public SpecificType2UInt64BitXor(int index, DataType inputTypes, int filterArg) {
    ...
    public void appendInitValue() {
        groupState.append(UInt64.UINT64_ZERO);
    }

    @Override
    public void resetToInitValue(int groupId) {
        groupState.set(groupId, UInt64.UINT64_ZERO);
    }
    ...
    public void accumulate(int groupId, Chunk chunk, int position) {
        ...
        groupState.set(groupId, beforeValue.bitXor(value));
    }
