 ...
 */
public class StringRuleGen implements IRuleGen {

    /**
     * (Math.abs(#key,start,物理总表数#.hashCode()) % 物理总表数).intdiv(每个库的分表数)
     * ((#key,1,物理总表数#).hashCode().abs().longValue() % 物理总表数).intdiv(每个库的分表数)
     */
    @Override
    public String generateDbRuleArrayStr(PartitionByType type, String idText, int start, int groups, int tablesPerGroup,
        ...
        if (type != PartitionByType.HASH) {
            throw new IllegalArgumentException("String column only support hash method");
        }
        ...
        /*
         * 这种写法SimpleRuleProcessor不认 rule.append("(Math.abs(");
         * rule.append("#").append(idText).append(",");
         * rule.append(start).append(","); rule.append(totalTables).append("#");
         * rule
         * .append(".hashCode()) % ").append(totalTables).append(").intdiv(")
         * .append(tablesPerGroup).append(")");
         */
        rule.append("((");
        rule.append("#").append(idText).append(",");
        rule.append(start).append(",");
        rule.append(totalTables).append("#");
        rule.append(").hashCode().abs().longValue() % ").append(totalTables).append(")");
    ...
    public String generateStandAloneDbRuleArrayStr(PartitionByType type, String idText, int start, int groups,
        ...
        rule.append(").hashCode().abs().longValue() % ").append(groups).append(")");
        return rule.toString();
    }
    ...
    public String generateTbRuleArrayStr(PartitionByType type, String idText, int start, int groups, int tablesPerGroup,
         ...
         * rule.append("#").append(idText).append(",");
         * rule.append(start).append(","); rule.append(totalTables).append("#");
         * rule.append(".hashCode()) % ").append(totalTables);
         */
        rule.append("((");
        rule.append("#").append(idText).append(",");
        rule.append(start).append(",");
    ...
    public String generateStandAloneTbRuleArrayStr(PartitionByType type, String idText, int start, int tablesPerGroup,
        ...
        rule.append(").hashCode().abs().longValue() % ").append(tablesPerGroup).append(")");
        return rule.toString();
    }
