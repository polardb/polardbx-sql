...
import static org.apache.calcite.sql.SqlKind.CHECK_SUM;

/**
 * OrcHash hash the CRC64 of each row using the hash function hash(x,y)= p + q * (x + y) + r * x * y
 * which the same as fast checker
 */
public class CheckSum extends Aggregator {

    public CheckSum() {
    }

    public CheckSum(int[] index, int filterArg) {
    ...
    public Aggregator getNew() {
        return new CheckSum(aggTargetIndexes, filterArg);
    }
