 ...
 */
public class ToDaysIterator extends AbstractDateIterator {

    public ToDaysIterator(DataType fieldType) {
        super(fieldType);
    }
    ...
    public boolean range(PartitionField from, PartitionField to, boolean lowerBoundIncluded,
        ...
        long dayNumberBegin = PartitionFunctionTimeCaculator.calDayNumber(
            minDatetime.getYear(),
            minDatetime.getMonth(),
            minDatetime.getDay()
        );
        long dayNumberEnd = PartitionFunctionTimeCaculator.calDayNumber(
            maxDatetime.getYear(),
            maxDatetime.getMonth(),
            maxDatetime.getDay()
        );
        this.count = dayNumberEnd - dayNumberBegin + 1;
    ...
    public Long next() {
        if (currentDatetime != null) {
            return PartitionFunctionTimeCaculator.calDayNumber(
