...
public class BooleanBlockEncoding implements BlockEncoding {
    ...
    public Block readBlock(SliceInput sliceInput) {
        ...
        for (int position = 0; position < positionCount; position++) {
            if (!valueIsNull[position]) {
                values[position] = sliceInput.readBoolean();
            }
        }
        return new BooleanBlock(0, positionCount, valueIsNull, values);
    }
    ...
    public void writeBlock(SliceOutput sliceOutput, Block block) {
        ...
        for (int position = 0; position < positionCount; position++) {
            if (!block.isNull(position)) {
                sliceOutput.writeBoolean(block.getBoolean(position));
            }
        }
    }
}
