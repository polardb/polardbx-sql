 ...
 * Created by zhuqiwei.
 */
public class ToMonthsIterator extends AbstractDateIterator {

    public ToMonthsIterator(DataType fieldType) {
        super(fieldType);
    }
    ...
    public boolean range(PartitionField from, PartitionField to, boolean lowerBoundIncluded,
        ...
        long toMonthsMin = PartitionFunctionTimeCaculator.calToMonths(
            minDatetime.getYear(),
            minDatetime.getMonth(),
            minDatetime.getDay()
        );
        long toMonthsMax = PartitionFunctionTimeCaculator.calToMonths(
            maxDatetime.getYear(),
            maxDatetime.getMonth(),
            maxDatetime.getDay()
        );
        this.count = toMonthsMax - toMonthsMin + 1;
        return true;
    }
    ...
    public boolean hasNext() {
        ...
        MysqlDateTime newDateTime = PartitionFunctionTimeCaculator.addInterval(
            currentDatetime,
            MySQLIntervalType.INTERVAL_MONTH,
    ...
    public Long next() {
        if (currentDatetime != null) {
            return PartitionFunctionTimeCaculator.calToMonths(
