...
public class UInt64SegementArrayList implements SegmentArrayList {
    private static final long INSTANCE_SIZE = ClassLayout.parseClass(BooleanSegmentArrayList.class).instanceSize();

    private static final int SEGMENT_SIZE = 1024;

    private List<UInt64[]> arrays;

    private int size;
    private int capacity;

    public UInt64SegementArrayList(int capacity) {
    ...
    private void grow() {
        arrays.add(new UInt64[SEGMENT_SIZE]);
