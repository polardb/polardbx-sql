 ...
 * @author dylan
 */
public class InformationSchema extends AbsSchema {

    public static final String NAME = "INFORMATION_SCHEMA";

    private static final InformationSchema INSTANCE = new InformationSchema();

    private InformationSchema() {
    }

    public static InformationSchema getInstance() {
        return INSTANCE;
    }

    @Override
    public Table getTable(String tableName) {
        SystemTableView.Row row = InformationSchemaViewManager.getInstance().select(tableName);
        if (row != null) {
