...
public class CreateTableWithGsiRecoverableTest extends DDLBaseNewDBTestCase {
    ...
    public CreateTableWithGsiRecoverableTest(String gsiDef) {
        this.primaryTableName = randomTableName(GSI_PRIMARY_TABLE_NAME, 8);
        this.createTable = buildCreateTableSqlDb(gsiDef).toString().replace(GSI_PRIMARY_TABLE_NAME, primaryTableName);
    }

    @Parameters(name = "{index}:gsiDef={0}")
    public static List<String[]> prepareDate() {
        List<String[]> params = ExecuteTableSelect.supportedGsiDef();
        if (CollectionUtils.size(params) < 3) {
            return params;
        }
        return params.subList(0, 3);
    }
    ...
    public static void beforeClass() throws SQLException {
        try (Connection tmpConnection = ConnectionManager.getInstance().newPolarDBXConnection()) {
            JdbcUtil.createPartDatabase(tmpConnection, "fail_point");
        }
    }
    ...
    public static void afterClass() throws SQLException {
        try (Connection tmpConnection = ConnectionManager.getInstance().newPolarDBXConnection()) {
            JdbcUtil.dropDatabase(tmpConnection, "fail_point");
        }
    }

    @Before
    public void before() throws SQLException {
        useDb(tddlConnection, "fail_point");
        this.failPointConnection = tddlConnection;
        clearFailPoints();

        JdbcUtil.executeUpdateSuccess(failPointConnection, "drop table if exists " + primaryTableName);
    }
    ...
    private void executeDDL() {
        checkCreateTableExecute(failPointConnection, HINT_CREATE_GSI + createTable, primaryTableName);

        // SHOW INDEX
        final ShowIndexChecker showIndexChecker = getShowIndexGsiChecker(failPointConnection, primaryTableName);
