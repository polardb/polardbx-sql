...
public class SpecialUpdateTest3 extends AutoCrudBasedLockTestCase {

    private final String HINT =
        "/*+TDDL:cmd_extra(ENABLE_COMPLEX_DML_CROSS_DB=true,COMPLEX_DML_WITH_TRX=FALSE)*/";

    public SpecialUpdateTest3() {
    ...
    /**
     * Update sharding key(varchar_test)
     */
    @Test
    public void updateMultiTableWithUpdateShardingKey() {
        ...
        for (int i = 0; i < 100; i++) {
            params.add(Lists.newArrayList(i, i, columnDataGenerator.varchar_testValue));
        }
        ...
        params.clear();
        for (int i = 0; i < 100; i++) {
            params.add(Lists.newArrayList(i, i, columnDataGenerator.varchar_testValueTwo));
        }
        executeBatchOnMysqlAndTddl(mysqlConnection, tddlConnection, insertSql, params);

        String sql = String
            .format(HINT + "update %s a, %s b set a.varchar_test=b.varchar_test where a.integer_test=b.integer_test",
                baseOneTableName,
                baseTwoTableName);
        ...
        sql = String.format("select pk, integer_test, varchar_test from %s", baseTwoTableName);
        selectContentSameAssert(sql, null, mysqlConnection, tddlConnection);
    }
}
