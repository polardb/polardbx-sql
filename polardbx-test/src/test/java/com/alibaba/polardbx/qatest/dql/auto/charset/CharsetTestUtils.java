...
public class CharsetTestUtils {
    ...
    public static List<byte[]> toUtf8BytesList(List<byte[]> from, CharsetName charsetName) {
        Preconditions.checkNotNull(from);
        CharsetHandler charsetHandler = CharsetFactory.INSTANCE.createCharsetHandler(charsetName);
        return from.stream()
            .map(charsetHandler::decode)
            .map(CharsetFactory.DEFAULT_CHARSET_HANDLER::encodeWithReplace)
            .map(Slice::getBase)
            .map(byte[].class::cast)
