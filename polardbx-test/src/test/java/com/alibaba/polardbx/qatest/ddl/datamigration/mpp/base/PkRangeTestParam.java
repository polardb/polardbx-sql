package com.alibaba.polardbx.qatest.ddl.datamigration.mpp.base;
...
//            }
            ...
            if (physicalBackfill) {
                ...
                if (maxPhysicalBackfillTaskRecordNum < 0) {
                    throw new TddlRuntimeException(ErrorCode.ERR_DDL_JOB_ERROR,
                        String.format(
                            "expected physical backfill record occur in show ddl engine status, but now only %d",
                            maxPhysicalBackfillTaskRecordNum));
                }
            } else {
                if (maxPkRangeBackfillTaskRecordNum < 2) {
                    throw new TddlRuntimeException(ErrorCode.ERR_DDL_JOB_ERROR,
                        String.format("expected pk range record occur in show ddl engine status, but now only %d",
                            maxPkRangeBackfillTaskRecordNum));
                }
...
//        result.addColumn("DDL_STMT", DataTypes.StringType);
    ...
    public static void controlTest(final Log logger, PkRangeTestParam pkRangeTestParam, Connection connection)
        ...
        if (!DdlStateCheckUtil.waitTillDdlDone(logger, connection, jobId, originalTableName,
            expectedLocalIndexConcurrency,
            "alter table",
            false)) {
            throw new TddlRuntimeException(ErrorCode.ERR_DDL_JOB_ERROR,
                String.format("wait addGsiDdl done timeout for %s", msg));
