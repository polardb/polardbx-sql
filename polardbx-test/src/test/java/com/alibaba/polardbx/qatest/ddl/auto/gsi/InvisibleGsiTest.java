 ...
 * @author zhuqiwei
 */
public class InvisibleGsiTest extends BaseAutoPartitionNewPartition {
    ...
    public void testCreateTableAndInvisibleGsi() throws Exception {
        ...
        final String createTable = MessageFormat.format("create table {0}"
                ...
                + "id int,"
                + "name varchar(20),"
                + "global index {1} (`name`) partition by hash(name) invisible,"
                + "unique global index {2} (`name`) partition by hash(name) invisible,"
                + "clustered index {3} (`name`) partition by hash(name) invisible"
                + ")"
    ...
    public void testAlterTableAlterVisibility() throws Exception {
        ...
        final String createTable = MessageFormat.format("create table {0}"
                ...
                + "id int,"
                + "name varchar(20),"
                + "global index {1} (`name`) partition by hash(name) visible,"
                + "unique global index {2} (`name`) partition by hash(name) visible,"
                + "clustered index {3} (`name`) partition by hash(name) visible"
                + ")"
                + "partition by hash(id)",
            TABLE_NAME, GSI_NAME, UGSI_NAME, CGSI_NAME);
    ...
    public void testCreateInvisibleGsi() throws Exception {
        ...
        final String createTable = MessageFormat.format("create table {0}"
            ...
            + "name varchar(20)"
            + ")"
            + "partition by hash(id)", TABLE_NAME);
        ...
        //create invisible gsi
        JdbcUtil.executeUpdateSuccess(tddlConnection, MessageFormat.format(
            "create global index {0} on {1} (name) partition by hash(name) invisible",
            GSI_NAME,
            TABLE_NAME
        ));
        JdbcUtil.executeUpdateSuccess(tddlConnection, MessageFormat.format(
            "create unique global index {0} on {1} (name) partition by hash(name) invisible",
            UGSI_NAME,
            TABLE_NAME
        ));
        JdbcUtil.executeUpdateSuccess(tddlConnection, MessageFormat.format(
            "create clustered index {0} on {1} (name) partition by hash(name) invisible",
            CGSI_NAME,
            TABLE_NAME
        ));
    ...
    public void testAlterTableAddInvisibleGsi() throws Exception {
        ...
        //create invisible gsi
        JdbcUtil.executeUpdateSuccess(tddlConnection, MessageFormat.format(
            "alter table {0} add global index {1}(name) partition by hash(name) invisible",
            TABLE_NAME,
            GSI_NAME
        ));
        JdbcUtil.executeUpdateSuccess(tddlConnection, MessageFormat.format(
