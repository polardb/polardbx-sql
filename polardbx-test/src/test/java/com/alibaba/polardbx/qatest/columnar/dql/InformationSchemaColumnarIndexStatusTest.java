...
public class InformationSchemaColumnarIndexStatusTest extends ReadBaseTestCase {
    ...
    private static final String TABLE_3 = "test_columnar_index_status_3";
    private static final String TABLE_4 = "test_columnar_index_status_4";
    ...
    private static final String INFORMATION_SCHEMA_SQL = "select * from information_schema.columnar_index_status "
    ...
    private static void prepareData() throws SQLException {
        try (Connection tddlConnection = ConnectionManager.getInstance().getDruidPolardbxConnection()) {
            ...
            for (String table : TABLES) {
                ...
                ColumnarUtils.createColumnarIndex(tddlConnection, "col_" + table, table, "id", "id", PART_COUNT);
            }
            ...
            waitForRowCountEquals(tddlConnection, TABLE_4, "col_" + TABLE_4);
    ...
    private void checkSingleTable(ResultSet rs, String tableName, long csvFileCount, long csvRowCount,
        ...
        if (csvFileCount > 0 && delFileCount > 0) {
            Assert.assertEquals(
                ...
                rs.getLong("ORC_ROW_COUNT") + rs.getLong("CSV_ROW_COUNT") - rs.getLong("DEL_ROW_COUNT")
            );
        }
