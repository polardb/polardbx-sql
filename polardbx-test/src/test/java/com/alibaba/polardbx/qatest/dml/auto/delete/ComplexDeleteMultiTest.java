...
public class ComplexDeleteMultiTest extends AutoCrudBasedLockTestCase {
    ...
    public static List<String[]> prepareData() {
        final List<String[]> tableNames = Arrays.asList(
            new String[][] {
                //相同库表会直接下推，不走LogicalModifyHandler
    ...
    public ComplexDeleteMultiTest(String baseOneTableName, String baseTwoTableName) {
    ...
    public void complexDeleteAll() throws Exception {
        String sql = HINT + "delete  b.* from " + baseOneTableName + " a, " + baseTwoTableName + " b where a.pk = b.pk";
    ...
    public void complexDeleteSomeTable1() throws Exception {
        String sql = HINT + "delete b.* from " + baseOneTableName + " a, " + baseTwoTableName
    ...
    public void complexDeleteWithBroadcastTest_exception() throws Exception {
        String sql = HINT + "delete a.* from " + baseOneTableName + " a, " + baseTwoTableName
            + " b where a.pk = b.pk and a.date_test < now()";
    ...
    public void complexdeleteSome1() throws Exception {
        ...
        String sql = HINT + "DELETE " + " a.* , b.* FROM " + baseOneTableName + " a , " + baseTwoTableName + " b "
    ...
    public void deleteWhereTest() throws Exception {
        String sql = HINT + String.format("delete from %s,%s using %s,%s where %s.pk=%s.pk",
            baseOneTableName, baseTwoTableName, baseOneTableName,
    ...
    public void deleteWhereAliasTest() throws Exception {
        String sql = HINT + String.format("delete from a,b using %s a,%s b where a.pk=b.pk",
            baseOneTableName, baseTwoTableName);
        executeOnMysqlAndTddl(mysqlConnection, tddlConnection, sql, null, true);
    ...
    public void deleteLeftJoinTest() throws Exception {
        String sql = HINT + String.format("delete from %s,%s using %s left join %s on %s.pk=%s.pk",
            baseOneTableName, baseTwoTableName, baseOneTableName,
            baseTwoTableName, baseOneTableName, baseTwoTableName);
    ...
    public void deleteLeftJoinAliasTest() throws Exception {
        String sql = HINT + String.format("delete from a,b using %s a left join %s b on a.pk=b.pk",
            baseOneTableName, baseTwoTableName);
        executeOnMysqlAndTddl(mysqlConnection, tddlConnection, sql, null, true);
        sql = String.format("select * from %s", baseOneTableName);
        selectContentSameAssert(sql, null, mysqlConnection,
