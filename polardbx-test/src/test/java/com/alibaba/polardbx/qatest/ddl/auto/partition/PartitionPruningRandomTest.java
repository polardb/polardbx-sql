 ...
 * @author chenghui.lch
 */
public class PartitionPruningRandomTest extends PartitionTestBase {

    protected static final Log log = LogFactory.getLog(PartitionPruningRandomTest.class);
    ...
    static {
        ...
        complexExprSampleList.add("(a,c) in ((1000,300),(a,400))");
    ...
    protected static class GenRandomCtx {
        ...
        protected int maxPredWidth = 5;
        protected int opPredCount = 16;
        protected AtomicInteger currOpPredCount = new AtomicInteger(0);

    }

    public PartitionPruningRandomTest() {
        super();
    }
    ...
    public void setUpEnv() {
        try {
            String createTbl = initCreateTableSql();
    ...
    public void runTest() {
        ...
        for (int i = 0; i < sqlCnt; i++) {
            ...
            String logMsg = String.format("rngSql: \n%s;\n\n", sql);
            log.info(logMsg);
            runRandomSql(sql);
        }
    }
    ...
    protected String genRandomSql() {
        GenRandomCtx ctx = new GenRandomCtx();
        String rndSqlPred = genRandPredExpr(0, ctx);
        String rndSql = String.format("select a,b,c,d from %s where %s order by a,b,c", testTableName, rndSqlPred);
        return rndSql;
    }
    ...
    protected String genRandOpExpr(GenRandomCtx randomCtx) {
        ...
        int complexExprFlag = randomInt(7);
