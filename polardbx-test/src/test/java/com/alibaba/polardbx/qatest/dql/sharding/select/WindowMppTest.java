...
public class WindowMppTest extends ReadBaseTestCase {
    private static final String TABLE_NAME = "drds_mpp_test_window";
    private static final String MPP_HINT = "/*+TDDL: ENABLE_MPP = true ENABLE_MASTER_MPP = true WORKLOAD_TYPE = AP*/ ";
    ...
    public void testUnbounded() {
        String sql = String
            .format(MPP_HINT + "select *,sum(id2) over (partition by id1),sum(id3) over (partition by id1) from "
                + TABLE_NAME);
    ...
    public void testRowSliding() {
        String sql = String.format(MPP_HINT + "select *,\n"
    ...
    public void testJoin() throws Exception {
        String sql = String.format(
            MPP_HINT
                + "select * from (select sum(id2) over (partition by id1 order by id2 RANGE BETWEEN UNBOUNDED PRECEDING AND 1 FOLLOWING) w1 from "
                + TABLE_NAME + ") a "
    ...
    public void testCurrentRowSumId2Desc() throws Exception {
        String sql = String.format(
            MPP_HINT
                + "select *, sum(id2) over (partition by id1 order by id2 desc range between current row and UNBOUNDED FOLLOWING) ow2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSumId2Asc() throws Exception {
        String sql = String.format(
            MPP_HINT
                + "select *, sum(id2) over (partition by id1 order by id2 asc range between current row and UNBOUNDED FOLLOWING) ow2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSumId3DescUnboundedFollowing() throws Exception {
        String sql = String.format(
            MPP_HINT
                + "select *, sum(id3) over (partition by id1 order by id2 desc range between current row and UNBOUNDED FOLLOWING) ow2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSumId3AscUnboundedFollowing() throws Exception {
        String sql = String.format(
            MPP_HINT
                + "select *, sum(id3) over (partition by id1 order by id2 asc range between current row and UNBOUNDED FOLLOWING) ow2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSumId2DescNoFrame() throws Exception {
        String sql = String.format(
            MPP_HINT + "select *, sum(id2) over (partition by id1 order by id2 desc) ow2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSumId2AscNoFrame() throws Exception {
        String sql = String.format(
            MPP_HINT + "select *, sum(id2) over (partition by id1 order by id2 asc) ow2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSumId3DescNoFrame() throws Exception {
        String sql = String.format(
            MPP_HINT + "select *, sum(id3) over (partition by id1 order by id2 desc) ow2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSumId3AscNoFrame() throws Exception {
        String sql = String.format(
            MPP_HINT + "select *, sum(id3) over (partition by id1 order by id2 asc) ow2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSum1EmptyOver() throws Exception {
        String sql = String.format(
            MPP_HINT + "select *, sum(id1) over () ow1 from  "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSum2EmptyOver() throws Exception {
        String sql = String.format(
            MPP_HINT + "select *, sum(id2) over () ow1 from  "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSum3EmptyOver() throws Exception {
        String sql = String.format(
            MPP_HINT + "select *, sum(id3) over () ow1 from  "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testCurrentRowSumSumEmptyOver() throws Exception {
        String sql = String.format(
            MPP_HINT + "select *, sum(id1) over () ow1,sum(id1) over () aw2 from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testWindowFunctionUseMultiTbForAggTogether() throws Exception {
        String sql = MPP_HINT + "select char_test"
    ...
    public void testSumType1() throws Exception {
        String sql =
            MPP_HINT + "select *,sum(decimal_test) over (PARTITION BY pk) from select_base_one_multi_db_one_tb t;";
    ...
    public void testSumType2() throws Exception {
        String sql =
            MPP_HINT + "select *,sum(double_test) over (PARTITION BY pk) from select_base_one_multi_db_one_tb t;";
    ...
    public void testSumType3() throws Exception {
        String sql =
            MPP_HINT + "select *,sum(integer_test) over (PARTITION BY pk) from select_base_one_multi_db_one_tb t;";
    ...
    public void testLeadLagValue() {
        String sql = String
            .format(MPP_HINT
                + "select *,lag(id2,2,'abc') over(order by id1,id2),lead(id2,2,'abc') over(order by id1,id2) from "
                + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
    ...
    public void testNTileValue() {
        String sql = String
            .format(
                "select *,ntile(4) over (PARTITION BY id1 ORDER BY id2 desc ROWS BETWEEN 1 PRECEDING AND 1 following),ntile(4) over (PARTITION BY id1 ORDER BY id2) from "
                    + TABLE_NAME);
        JdbcUtil.executeSuccess(tddlConnection, sql);
    }
}
