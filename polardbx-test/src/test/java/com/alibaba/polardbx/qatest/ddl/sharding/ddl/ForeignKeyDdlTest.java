...
public class ForeignKeyDdlTest extends DDLBaseNewDBTestCase {
    ...
    private static final String dataBaseName = "ForeignKeyDdlDB";
...
//        doClearDatabase();
        ...
        for (String partitionDef1 : PART_DEFS) {
            ...
            for (String partitionDef2 : PART_DEFS) {
                if (partitionDef2.contains("`b`,`c`") || partitionDef2.contains("single") || partitionDef2.contains(
                    "broadcast")) {
                    // show create table has auto-shared-key | has no single
                    continue;
                }
                ...
                assertEquals(String.format("CREATE TABLE `device` (\n"
                    + "\t`a` int(11) NOT NULL AUTO_INCREMENT BY GROUP,\n"
                    + "\t`b` int(11) NOT NULL,\n"
                    + "\t`c` int(11) NOT NULL,\n"
                    + "\t`d` int(11) NOT NULL,\n"
                    + "\tPRIMARY KEY (`a`),\n"
        ...
        for (String partitionDef1 : PART_DEFS) {
            ...
            for (String partitionDef2 : PART_DEFS) {
                ...
                for (String partitionDef3 : PART_DEFS) {
                    if (partitionDef3.equals(partitionDef2)) {
                        continue;
                    }
                    ...
                    if (partitionDef3.equals(partitionDef1) && (partitionDef3.equalsIgnoreCase("SINGLE")
    ...
    public void CreateTableDifferentCharsetWithFkReferred() throws SQLException {
        ...
        JdbcUtil.executeUpdateSuccess(tddlConnection,
            String.format(CREATE_CHARSET_P, "DEFAULT CHARSET = utf8mb4 dbpartition by hash(`a`)"));
        JdbcUtil.executeUpdateFailed(tddlConnection,
            String.format(CREATE_CHARSET_C, "DEFAULT CHARSET = utf8 dbpartition by hash(`a`)"),
            "Cannot add foreign key constraint due to different charset or collation");

        JdbcUtil.executeUpdateSuccess(tddlConnection,
            String.format(CREATE_CHARSET_C, "dbpartition by hash(`a`)"));

        dropTableIfExists("charset_c");

        JdbcUtil.executeUpdateSuccess(tddlConnection,
            String.format(CREATE_CHARSET_C, "DEFAULT CHARSET = UTF8MB4 dbpartition by hash(`a`)"));

        dropTableIfExists("charset_c");
        dropTableIfExists("charset_p");

        JdbcUtil.executeUpdateSuccess(tddlConnection, String.format(CREATE_CHARSET_P,
            "DEFAULT CHARSET = utf8mb4  DEFAULT COLLATE = utf8mb4_general_ci dbpartition by hash(`a`)"));
        JdbcUtil.executeUpdateFailed(tddlConnection, String.format(CREATE_CHARSET_C,
                "DEFAULT CHARSET = utf8mb4  DEFAULT COLLATE = utf8mb4_unicode_ci dbpartition by hash(`a`)"),
            "Cannot add foreign key constraint due to different charset or collation");
