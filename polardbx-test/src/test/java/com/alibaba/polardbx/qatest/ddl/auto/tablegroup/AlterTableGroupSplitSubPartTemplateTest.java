...
@NotThreadSafe
public class AlterTableGroupSplitSubPartTemplateTest extends AlterTableGroupSubPartitionTest {

    public AlterTableGroupSplitSubPartTemplateTest(PartitionRuleInfo partitionRuleInfo) {
        ...
        for (int i = 1; i < partitions.length; i++) {
            for (int j = 1; j < subPartitions.length; j++) {
                partitionRuleInfos.add(new PartitionRuleInfo(
                    ...
                    alterTableGroup + " " + splitPartitionCommands.get(partitions[i]),
                    "p2"
                ));
                partitionRuleInfos.add(new PartitionRuleInfo(
                    partStrategies[i], initDataType, ignoreInit,
                    tPartitionRules.get(partitions[i] + subPartitions[j]),
                    alterTable + " " + splitPartitionCommands.get(partitions[i]),
                    "p2"
                ));
                partitionRuleInfos.add(new PartitionRuleInfo(
                    partStrategies[i], initDataType, ignoreInit,
                    tPartitionRules.get(partitions[i] + subPartitions[j]),
                    alterTableGroup + " " + splitSubPartitionCommands.get(subPartitions[j]),
                    "p2sp2"
                ));
                partitionRuleInfos.add(new PartitionRuleInfo(
                    partStrategies[i], initDataType, ignoreInit,
                    tPartitionRules.get(partitions[i] + subPartitions[j]),
                    alterTable + " " + splitSubPartitionCommands.get(subPartitions[j]),
                    "p2sp2"
                ));
            }
        }
