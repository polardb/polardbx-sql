...
public class ShowCreateTableAutoIncrementValTest extends DDLBaseNewDBTestCase {
    ...
    private List<String> tbSqls = ImmutableList.of(
        "create table group_seq_tb_partition("
            ...
            + ") dbpartition by hash(id) tbpartition by hash(id) tbpartitions 16",
    ...
    public void testShowAutoIncrementGroupSeqTmSeq() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            String result4 = null;
            try (ResultSet rs = JdbcUtil.executeQuerySuccess(tddlConnection, "show create table " + tbNames.get(3))) {
                assertThat(rs.next()).isTrue();
            ...
            String result6 = null;
            try (ResultSet rs = JdbcUtil.executeQuerySuccess(tddlConnection,
                "show full create table " + tbNames.get(5))) {
                assertThat(rs.next()).isTrue();
    ...
    public void testShowNoSequence() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            //partition table
            prepareData2(tbNames.get(6), randomNumber, idx);
            String result = null;
            try (ResultSet rs = JdbcUtil.executeQuerySuccess(tddlConnection,
                "show full create table " + tbNames.get(6))) {
                assertThat(rs.next()).isTrue();
                result = rs.getString("Create Table");
            } catch (SQLException ignore) {
            }
            ...
            //single table
            prepareData2(tbNames.get(7), randomNumber, idx);
            String result2 = null;
            try (ResultSet rs = JdbcUtil.executeQuerySuccess(tddlConnection,
                "show create table " + tbNames.get(7))) {
                assertThat(rs.next()).isTrue();
                result2 = rs.getString("Create Table");
            } catch (SQLException ignore) {
            }
            ...
            //broadcast table
            prepareData2(tbNames.get(8), randomNumber, idx);
            String result3 = null;
            try (ResultSet rs = JdbcUtil.executeQuerySuccess(tddlConnection,
