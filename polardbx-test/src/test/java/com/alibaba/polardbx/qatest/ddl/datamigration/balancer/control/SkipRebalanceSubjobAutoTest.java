...
public class SkipRebalanceSubjobAutoTest extends DDLBaseNewDBTestCase {
    ...
    public SkipRebalanceSubjobAutoTest(boolean enableChangeSet) {
    ...
    @Test
    public void testSkipRebalanceSubjobDrainNode() throws SQLException, InterruptedException {
        // create table
    ...
    @Test
    @Ignore
    public void testSkipRebalanceSubjob() throws SQLException, InterruptedException {
        // create table
        for (int i = 0; i < TABLE_COUNT; ++i) {
            String tbName = TABLE_PREFIX + i;
            JdbcUtil.executeUpdateSuccess(tddlConnection, String.format(DROP_TABLE_SQL, tbName));
    ...
    private static String prepareDrainNodeCommand(Connection connection) throws SQLException {
        ...
        ResultSet rs = JdbcUtil.executeQuery(sql, connection);
        while (rs.next()) {
            if (rs.getString("GROUP").equalsIgnoreCase("SKIPREBALANCESUBJOBAUTOTEST_P00001_GROUP")) {
