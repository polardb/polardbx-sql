...
public class HybridBushyJoinTest extends AutoReadBaseTestCase {
    ...
    public HybridBushyJoinTest(String g1tb1, String g1tb2, String g1tb3, String g1tb4,
    ...
    public void InnerThreeBushyJoin() {
        for (String[] group : groupList) {
            String sql = hint + "select * from " + group[0] + " c , " + group[1] + " a, " + group[3] + " b " +
    ...
    public void InnerFourBushyJoin() {
        for (String[] group : groupList) {
            String sql =
                hint + "select * from " + group[0] + " c , " + group[1] + " a, " + group[2] + " b, " + group[3] + " d "
                    +
    ...
    public void InnerThreeLeftBushyJoin() {
        for (String[] group : groupList) {
            String sql = hint + "select * from " + group[0] + " a left join " +
    ...
    public void InnerThreeRightBushyJoin() {
        for (String[] group : groupList) {
            String sql = hint + "select * from " + group[0] + " a right join " +
                " ( select (case when t1.pk+t2.pk < 500 then t1.pk+t2.pk else null end) as pk, t1.integer_test " +
                "   from " + group[1] + " t1, " + group[2] + " t2 where t1.pk = t2.pk ) b " +
                " on a.pk = b.pk ";
            selectContentSameAssert(sql, null, mysqlConnection, tddlConnection);
    ...
    public void InnerFourLeftBushyJoin() {
        for (String[] group : groupList) {
            String sql = hint + "select * from \n" +
                "( select (case when pk > 750 then null else pk end ) as pk from " + group[0] + ") t0 \n" +
                "left join\n" +
                "( select (case when pk < 250 then null else pk end ) as pk from " + group[1]
                + ") t1 on t0.pk = t1.pk\n" +
                "left join\n" +
                "( select (case when pk > 250 and pk < 500 then null else pk end ) as pk from " + group[2]
                + ") t2 on t1.pk = t2.pk\n" +
                "left join\n" +
                "( select (case when pk > 750 then null else pk end ) as pk from " + group[3] + ") t3 on t2.pk = t3.pk";
            selectContentSameAssert(sql, null, mysqlConnection, tddlConnection);
        }
    }
    ...
    public void InnerEightLeftBushyJoin() {
        for (String[] group : groupList) {
            ...
            String sql = hint + "select * from (" + leftSql + ") t1 left join (" + rightql + ") t2 on t1.pk = t2.pk";
            selectContentSameAssert(sql, null, mysqlConnection, tddlConnection);
        }
    }
    ...
    public void InnerEightRightBushyJoin() {
        for (String[] group : groupList) {
            ...
            String sql = hint + "select * from (" + leftSql + ") t1 right join (" + rightql + ") t2 on t1.pk = t2.pk";
            selectContentSameAssert(sql, null, mysqlConnection, tddlConnection);
        }
    }
    ...
    public void InnerFiveBushyJoin() {
        for (String[] group : groupList) {
            String sql = hint + "select * from " + group[0] + " a, "
                + group[1] + " b, "
                + group[2] + " c, "
                + group[3] + " d, "
                + group[0] + " e "
    ...
    public void Inner16BushyJoinDifferentGroup() {
        String sql = hint + "select * from " + groupList[0][0] + " t00, "
            + groupList[1][0] + " t10, "
            + groupList[2][0] + " t20, "
            + groupList[3][0] + " t30, "
            + groupList[0][1] + " t01, "
     ...
     * @since 5.3.7
     */
    @Ignore("PushJoinRule fixed, do not need bushy join rule ")
    public void Inner3BushyJoinParenthesesTest() {
        for (int i = 1; i <= 3; i++) {
            String sql = hint + "select * from " + groupList[i][0] + " a, ( "
                + groupList[i][1] + " b join "
                + groupList[i][2] + " c  on b.pk = c.pk ) where a.pk = c.pk";
            selectContentSameAssert(sql, null, mysqlConnection, tddlConnection);
            testBushyJoinRuleHasBeenUsed(sql);
        }
    }
    ...
    public void InnerLeft4BushyJoinParenthesesTest() {
        String sql =
            hint + "select * from " + groupList[3][0] + " t1, " + groupList[2][1] + " t2, " + groupList[3][2] + " t3 " +
    ...
    public void Inner3BushyJoinBroadcastTest() {
        String sql =
            hint + "select * from " + groupList[3][0] + " t1, " + groupList[2][1] + " t2, " + groupList[4][1] + " t3 " +
    ...
    public void Inner4BushyJoinBroadcastTest() {
        String sql = hint + "select * from " + groupList[3][0] + " t1, "
            + groupList[2][1] + " t2, "
            + groupList[4][1] + " t3, "
            + groupList[4][2] + " t4 " +
