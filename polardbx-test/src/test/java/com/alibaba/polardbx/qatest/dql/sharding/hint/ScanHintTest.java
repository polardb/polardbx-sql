...
public class ScanHintTest extends ReadBaseTestCase {
    ...
    public void testPointSelect() {
        ...
        for (int i = 0; i < pointSelectHints.length; i++) {
            switch (i) {
            case 2:
                JdbcUtil.executeQueryFaied(tddlConnection, pointSelectSqlBuilder.apply(i, group1[1]), failed);
                JdbcUtil.executeSuccess(tddlConnection, pointSelectSqlBuilder.apply(i, group2[1]));
                JdbcUtil.executeQueryFaied(tddlConnection, pointSelectSqlBuilder.apply(i, group3[1]), failed);
    ...
    public void testMultiTable() {
        ...
        final String[] multiTableHints = new String[] {
            ...
            "/*+TDDL:SCAN('%s, %s')*/",
            "/*+TDDL:SCAN('%s, %s, %s')*/",
            "/*+TDDL:SCAN('%s a, %s b, %s c', node='0,2')*/",
        ...
        for (int i = 0; i < multiTableHints.length; i++) {
            ...
            case 3:
                ...
                JdbcUtil.executeQueryFaied(tddlConnection,
                    joinBuilder.apply(i, Lists.newArrayList(group1[1], group1[2], group1[3])), failed);
                // multi db one tb
                ...
                JdbcUtil.executeQueryFaied(tddlConnection,
                    joinBuilder.apply(i, Lists.newArrayList(group1[2], group2[2], group2[0])), failed);
                // broadcast table
                JdbcUtil.executeSuccess(tddlConnection,
        ...
        for (int i = 0; i < multiTableHints.length; i++) {
            ...
            case 3:
                // single table
                JdbcUtil.executeQueryFaied(tddlConnection,
                    subqueryBuilder.apply(i, Lists.newArrayList(group1[1], group1[2], group1[3])), failed);
                // multi db one tb
                JdbcUtil.executeSuccess(tddlConnection,
                ...
                // multi db one tb + single table
                JdbcUtil.executeQueryFaied(tddlConnection,
                    subqueryBuilder.apply(i, Lists.newArrayList(group1[2], group2[2], group2[0])), failed);
                // broadcast table
                JdbcUtil.executeSuccess(tddlConnection,
