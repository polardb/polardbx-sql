...
public class BinlogCommandAccessor extends AbstractAccessor {
    ...
    public List<BinlogCommandRecord> getBinlogCommandRecordById(long id) {
        try {
            ...
            return MetaDbUtil
                .query(QUERY_COMMAND_BY_ID, selectParams, BinlogCommandRecord.class, connection);
        } catch (Exception e) {
    ...
    public List<BinlogCommandRecord> getBinlogCommandRecordsByTime(long time) {
        try {
            Map<Integer, ParameterContext> selectParams = Maps.newHashMap();
            MetaDbUtil.setParameter(1, selectParams, ParameterMethod.setLong, time);
            return MetaDbUtil
                .query(QUERY_INITIAL_STATUS_COMMAND_BY_TIME, selectParams, BinlogCommandRecord.class, connection);
        } catch (Exception e) {
            MetaDbLogUtil.META_DB_LOG
                .error("Failed to query the system table '" + BINLOG_POLARX_COMMAND_TABLE + "'", e);
            throw new TddlRuntimeException(ErrorCode.ERR_GMS_ACCESS_TO_SYSTEM_TABLE, e, "query",
    ...
    public List<BinlogCommandRecord> getBinlogCommandRecordByTypeAndStatus(String commandType, int status) {
        try {
            Map<Integer, ParameterContext> selectParams = Maps.newHashMap();
            ...
            return MetaDbUtil
                .query(QUERY_COMMAND_BY_TYPE_AND_STATUS, selectParams, BinlogCommandRecord.class, connection);
        } catch (Exception e) {
            MetaDbLogUtil.META_DB_LOG
                .error("Failed to query the system table '" + BINLOG_POLARX_COMMAND_TABLE + "'", e);
            throw new TddlRuntimeException(ErrorCode.ERR_GMS_ACCESS_TO_SYSTEM_TABLE, e, "query",
    ...
    public List<BinlogCommandRecord> getBinlogCommandRecordByTypeAndCmdId(String commandType, String commandId) {
        try {
            Map<Integer, ParameterContext> selectParams = Maps.newHashMap();
            MetaDbUtil.setParameter(1, selectParams, ParameterMethod.setString, commandType);
            MetaDbUtil.setParameter(2, selectParams, ParameterMethod.setString, commandId);
            return MetaDbUtil
                .query(QUERY_COMMAND_BY_TYPE_AND_CMD_ID, selectParams, BinlogCommandRecord.class, connection);
        } catch (Exception e) {
            MetaDbLogUtil.META_DB_LOG
                .error("Failed to query the system table '" + BINLOG_POLARX_COMMAND_TABLE + "'", e);
            throw new TddlRuntimeException(ErrorCode.ERR_GMS_ACCESS_TO_SYSTEM_TABLE, e, "query",
    ...
    public List<BinlogCommandRecord> getBinlogCommandRecordByType(String commandType) {
        try {
            Map<Integer, ParameterContext> selectParams = Maps.newHashMap();
            MetaDbUtil.setParameter(1, selectParams, ParameterMethod.setString, commandType);
            return MetaDbUtil
                .query(QUERY_COMMAND_BY_TYPE, selectParams, BinlogCommandRecord.class, connection);
        } catch (Exception e) {
            MetaDbLogUtil.META_DB_LOG
                .error("Failed to query the system table '" + BINLOG_POLARX_COMMAND_TABLE + "'", e);
            throw new TddlRuntimeException(ErrorCode.ERR_GMS_ACCESS_TO_SYSTEM_TABLE, e, "query",
    ...
    public int insertIgnoreBinlogCommandRecord(BinlogCommandRecord commandRecord) {
        try {
