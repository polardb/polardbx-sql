package com.alibaba.polardbx.common.utils.binlog;
...
public class JsonConversion {
    ...
    public static class Json_Value {
        ...
        public StringBuilder toJsonString(StringBuilder buf, Charset charset) {
            ...
            case ARRAY:
                ...
                break;
            case DOUBLE:
                buf.append(Double.valueOf(m_double_value).toString().toLowerCase());
                break;
            case INT:
                buf.append(m_int_value.toString().toLowerCase());
                break;
            case UINT:
                buf.append(m_int_value.toString().toLowerCase());
                break;
            ...
            case OPAQUE:
                String text = null;
                if (m_field_type == MySQLStandardFieldType.MYSQL_TYPE_NEWDECIMAL.getId()) {
                    int precision = m_data.getInt8();
                    int scale = m_data.getInt8();
                    text = m_data.getDecimal(precision, scale).toPlainString();
                    buf.append(text);
                } else if (m_field_type == MySQLStandardFieldType.MYSQL_TYPE_TIME.getId()) {
                ...
                } else if (m_field_type == MySQLStandardFieldType.MYSQL_TYPE_DATE.getId()
                    || m_field_type == MySQLStandardFieldType.MYSQL_TYPE_DATETIME.getId()
                    || m_field_type == MySQLStandardFieldType.MYSQL_TYPE_TIMESTAMP.getId()) {
                    long packed_value = m_data.getLong64();
                    if (packed_value == 0) {
    ...
    private static void appendNumber2(StringBuilder builder, int d) {
        if (d >= 10) {
    ...
    private static void appendNumber3(StringBuilder builder, int d) {
        if (d >= 100) {
    ...
    private static void appendNumber4(StringBuilder builder, int d) {
        if (d >= 1000) {
