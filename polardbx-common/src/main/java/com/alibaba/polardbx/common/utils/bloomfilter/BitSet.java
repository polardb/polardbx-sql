...
public class BitSet {
    ...
    public String toBitString() {
        StringBuilder stringBuilder = new StringBuilder(data.length * 64);
        for (long l : data) {
            stringBuilder.append(Long.toBinaryString(l));
        }
        return stringBuilder.toString();
    }

    public static double getUsage(long[] data) {
        double totalBits = data.length * 64;
        double ones = 0d;
        for (long l : data) {
            ones += Long.bitCount(l);
        }
