...
import org.apache.calcite.plan.volcano.PlannerTests.Phys2LeafRule;
...
import static org.apache.calcite.plan.volcano.PlannerTests.PHYS2_CALLING_CONVENTION;
...
public class VolcanoPlannerTest {
   ...
   * Tests transformation of a leaf from NONE to PHYS.
   */
  @Test public void testTransformLeaf_Debug() {
    VolcanoPlanner planner = new VolcanoPlanner();

    planner.addRelTraitDef(ConventionTraitDef.INSTANCE);

    planner.addRule(new PhysLeafRule());
    planner.addRule(new Phys2LeafRule());
    ...
    RelNode newRoot = result;
    RelNode newConvertedRoot =
    planner.changeTraits(
        newRoot,
        cluster.traitSetOf(PHYS2_CALLING_CONVENTION));
    planner.setRoot(newConvertedRoot);
    RelNode newResult = planner.chooseDelegate().findBestExp();
