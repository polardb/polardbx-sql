...
class PlannerTests {
  ...
  static final Convention PHYS2_CALLING_CONVENTION =
      new Convention.Impl("PHYS2", RelNode.class) {
        @Override public boolean canConvertConvention(Convention toConvention) {
          return true;
        }
  ...
  /** Relational expression with zero inputs and convention PHYS. */
  static class Phys2LeafRel extends TestLeafRel {
    Phys2LeafRel(RelOptCluster cluster, String label) {
      super(cluster, cluster.traitSetOf(PHYS2_CALLING_CONVENTION), label);
    }
    ...
    @Override public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
      assert traitSet.comprises(PHYS2_CALLING_CONVENTION);
      assert inputs.isEmpty();
      return this;
    }
  }
  ...
  /** Planner rule that converts {@link NoneLeafRel} to PHYS convention. */
  static class Phys2LeafRule extends RelOptRule {
    Phys2LeafRule() {
      super(operand(PhysLeafRel.class, any()));
    }

    @Override public Convention getOutConvention() {
      return PHYS2_CALLING_CONVENTION;
    }

    public void onMatch(RelOptRuleCall call) {
      PhysLeafRel leafRel = call.rel(0);
      call.transformTo(
          new Phys2LeafRel(leafRel.getCluster(), leafRel.label));
    }
  }
