...
public class RelFieldTrimmer implements ReflectiveVisitor {
  ...
  private static final ReflectUtil.MethodDispatcher<TrimResult> trimFieldsDispatcher = ReflectUtil.createMethodDispatcher(
      TrimResult.class,
  ...
  protected final TrimResult dispatchTrimFields(
      ...
      Set<RelDataTypeField> extraFields) {
    final TrimResult trimResult =
        trimFieldsDispatcher.invoke(this, rel, fieldsUsed, extraFields);
          ...
          new CorrelationReferenceFinder() {
            protected RexNode handle(RexFieldAccess fieldAccess) {
              ...
              if (v.getId().equals(correlation)
                ...
                final RexNode newV =
                    rexBuilder.makeCorrel(typeBuilder.build(), v.getId());
                if (old != new_) {
  ...
  public TrimResult trimFields(
    ...
    if (join instanceof SemiJoin) {
      relBuilder.semiJoin(join.getHints(), newConditionExpr);
