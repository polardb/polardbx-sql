...
public class RelDecorrelator implements ReflectiveVisitor {
  ...
  private static final ReflectUtil.MethodDispatcher<Frame> dispatcher =
      ReflectUtil.createMethodDispatcher(Frame.class, RelDecorrelator.class, "decorrelateRel",
          RelNode.class);
  ...
  public Frame getInvoke(RelNode r, RelNode parent) {
    final Frame frame = dispatcher.invoke(this, r);
    if (frame != null) {
      map.put(r, frame);
    }
    currentRel = parent;
  ...
  private boolean references(RexNode e, CorRef correlation) {
    ...
    case FIELD_ACCESS:
      ...
      if (f.getField().getIndex() == correlation.field
        ...
        if (((RexCorrelVariable) f.getReferenceExpr()).getId() == correlation.corr) {
          return true;
        }
      }
      // fall through
    ...
    private void onMatch2(
      ...
      LogicalCorrelate newCorrelate =
          LogicalCorrelate.create(leftInput, aggregate,
              correlate.getCorrelationId(), correlate.getRequiredColumns(), null, null,
          ...
          if (ref instanceof RexCorrelVariable) {
            ...
            if (mapFieldAccessToCorVar.containsKey(fieldAccess)) {
              ...
              final CorRef correlation =
                  new CorRef(var.getId(), fieldAccess.getField().getIndex(),
                      corrIdGenerator++);
