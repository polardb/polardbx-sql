...
public class SqlSetOption extends SqlAlter {
  ...
  /**
   * alias database name that have identifier name.
   */
  boolean alias;
  ...
  public SqlSetOption(SqlParserPos pos, String scope, SqlIdentifier name,
      SqlNode value, boolean alias, boolean session) {
    this(pos, scope, name, value);
    this.alias = alias;
    this.session = session;
  }

  public SqlSetOption(SqlParserPos pos, String scope, boolean session,
      int characteristic, int level) {
    super(pos, scope);
    this.session = session;
  ...
  private void unparseTransaction(SqlWriter writer) {
    writer.keyword("SET");
    if (session) {
      writer.keyword("SESSION");
    } else {
    ...
    case 1:
      writer.keyword("ISOLATION");
      writer.keyword("LEVEL");
      unparseIsolationLevel(writer);
      break;
    case 2:
      writer.keyword("READ");
      writer.keyword("ONLY");
  ...
  private void unparseIsolationLevel(SqlWriter writer) {
    switch (level) {
    case 1:
      writer.keyword("REPEATABLE");
      writer.keyword("READ");
      return;
    case 2:
      writer.keyword("READ");
      writer.keyword("COMMITTED");
      return;
    case 3:
      writer.keyword("READ");
      writer.keyword("UNCOMMITTED");
      return;
    case 4:
      writer.keyword("SERIALIZABLE");
      return;
    default:
      // do nothing.
    }
