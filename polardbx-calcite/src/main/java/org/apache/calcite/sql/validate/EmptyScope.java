...
class EmptyScope implements SqlValidatorScope {
  ...
  private void resolve_(final CalciteSchema rootSchema, List<String> names,
    ...
    final List<String> concat = names.size() > 1 ? Util.skipLast(names) : schemaNames;
    ...
    CalciteSchema.TableEntry entry =
        schema.getTable(tableName, nameMatcher.isCaseSensitive());
    if (entry == null) {
      entry = schema.getTableBasedOnNullaryFunction(tableName,
    ...
    if (entry != null) {
      ...
      resolved.found(namespace, false, this, path, ImmutableList.<String>of());
      return;
    }
