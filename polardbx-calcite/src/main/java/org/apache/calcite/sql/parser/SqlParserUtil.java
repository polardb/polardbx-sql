...
public final class SqlParserUtil {
  ...
  public static String strip(String s, String startQuote, String endQuote,
    ...
    if (startQuote != null) {
      ...
      s = Util.replace(s.substring(1, s.length() - 1), escape, endQuote);
    }
    switch (casing) {
    case TO_UPPER:
  ...
  public static ParsedCollation parseCollation(String in) {
    ...
    String localeStr = st.hasMoreTokens() ? st.nextToken() : "en_US";
    /**
     * For mysql there is no locale information in collation, use en_us as default.
     */
    String strength;
    if (st.countTokens() > 0) {
