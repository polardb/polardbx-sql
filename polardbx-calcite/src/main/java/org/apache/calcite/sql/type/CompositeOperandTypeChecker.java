...
public class CompositeOperandTypeChecker implements SqlOperandTypeChecker {
  ...
  public String getAllowedSignatures(SqlOperator op, String opName) {
    ...
    for (Ord<? extends SqlOperandTypeChecker> ord : Ord.zip(allowedRules)) {
      if (ord.i > 0) {
    ...
    case SEQUENCE:
      ...
      for (Ord<? extends SqlOperandTypeChecker> ord : Ord.zip(allowedRules)) {
        ...
        if (!((SqlSingleOperandTypeChecker) rule).checkSingleOperandType(
            ...
            0,
            false)) {
          if (callBinding.getValidator().isTypeCoercionEnabled() && SqlStdOperatorTable.isTypeCoercionEnable(callBinding.getOperator())) {
