...
public abstract class DelegatingScope implements SqlValidatorScope {
  ...
  public SqlQualified fullyQualify(SqlIdentifier identifier) {
    ...
    // fall through
    default: {
      if (identifier.names.get(identifier.names.size() - 1).equalsIgnoreCase(SqlFunction.NEXTVAL_FUNC_NAME)) {
        return SqlQualified.create(this, 1, null, identifier);
      }
      ...
      case 0:
        ...
        final SqlIdentifier prefix = identifier.getComponent(0, i);
        final SqlIdentifier suffix3 = identifier.getComponent(i, k + 1);
        if (this instanceof CatalogScope || this.parent == null) {
          throw validator.newValidationError(suffix3,
                  RESOURCE.columnNotFoundInTable(suffix3.toString(), prefix.toString()));
        } else {
          //run to here, we can make sure that the table is found, and then
          try {
            return this.parent.fullyQualify(identifier);
          } catch (CalciteContextException e) {
            if (e.getMessage().contains(RESOURCE.tableNameNotFound(prefix.toString()).ex().getMessage())) {
              throw validator.newValidationError(suffix3,
                      RESOURCE.columnNotFoundInTable(suffix3.toString(), prefix.toString()));
            }
            throw e;
