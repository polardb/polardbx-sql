...
public abstract class AbstractTypeCoercion implements TypeCoercion {
  ...
  protected boolean coerceOperandType(
    ...
    final SqlNode operand = nodeList.get(index);
    // Check it early.
  ...
  protected boolean needToCast(SqlValidatorScope scope, SqlNode node, RelDataType toType) {
    ...
    // No need to cast between approximate types.
    if (SqlTypeUtil.isApproximateNumeric(toType) && SqlTypeUtil.isApproximateNumeric(fromType)) {
      return false;
    }
    ...
    // the cast from int type to approximate type is guaranteed by java implicit cast mechanism. e.g. int i = 1; double d = i;
    if (SqlTypeUtil.isIntType(fromType) && SqlTypeUtil.isApproximateNumeric(toType)) {
      return false;
    }
  ...
  private SqlNode castTo(SqlNode node, RelDataType type) {
    // fix the invalid decimal type
    if(type.getSqlTypeName() == SqlTypeName.DECIMAL) {
      if(type.getPrecision() < type.getScale()) {
        type = factory.createSqlType(SqlTypeName.DECIMAL);
      }
    }
    return SqlStdOperatorTable.IMPLICIT_CAST.createCall(SqlParserPos.ZERO, node,
  ...
  public RelDataType commonTypeForBinaryComparison(RelDataType type1, RelDataType type2) {
    ...
    // string <cmp op> numeric
    if (SqlTypeUtil.isString(type1) && SqlTypeUtil.isNumeric(type2)) {
      return factory.createSqlType(SqlTypeName.DOUBLE);
    } else if (SqlTypeUtil.isNumeric(type2) && SqlTypeUtil.isString(type1)) {
      return factory.createSqlType(SqlTypeName.DOUBLE);
    }

    // datetime <cmp op> numeric
    if (SqlTypeUtil.isDatetime(type1) && SqlTypeUtil.isNumeric(type2)) {
      return type1;
    } else if (SqlTypeUtil.isNumeric(type1) && SqlTypeUtil.isDatetime(type2)) {
      return type2;
    }
