 ...
 * <p>
 * There is also a <em>secondary</em> family categorization which overlaps with
 * the primary categorization. It is used in type strategies for more specific
 * or more general categorization than the primary families. Secondary families
 * are never returned by RelDataType.getFamily().
 */
public enum SqlTypeFamily implements RelDataTypeFamily {
    ...
    STRING, APPROXIMATE_NUMERIC, EXACT_NUMERIC, DECIMAL, INTEGER, EXACT_INTEGER, EXACT_BIGINT, EXACT_BIGINT_UNSIGNED,
    DATETIME, DATETIME_INTERVAL,
    MULTISET, ARRAY, MAP, NULL, ANY, CURSOR, COLUMN_LIST, GEO, NUMERIC_BOOLEAN;

    static final long INSTANCE_SIZE = ClassLayout.parseClass(SqlTypeFamily.class).instanceSize();
    ...
    public Collection<SqlTypeName> getTypeNames() {
        switch (this) {
            ...
            case EXACT_INTEGER:
                return ImmutableList.of(SqlTypeName.TINYINT,
                        SqlTypeName.TINYINT_UNSIGNED,
                        SqlTypeName.SMALLINT,
                        SqlTypeName.SMALLINT_UNSIGNED,
                        SqlTypeName.MEDIUMINT,
                        SqlTypeName.MEDIUMINT_UNSIGNED,
                        SqlTypeName.INTEGER);
            case EXACT_BIGINT:
                return ImmutableList.of(SqlTypeName.INTEGER_UNSIGNED, SqlTypeName.BIGINT);
            case EXACT_BIGINT_UNSIGNED:
                return ImmutableList.of(SqlTypeName.BIGINT_UNSIGNED);
            case DATETIME:
            ...
            case NUMERIC_BOOLEAN:
                return ListUtils.union(SqlTypeName.NUMERIC_TYPES, SqlTypeName.BOOLEAN_TYPES);
            default:
    ...
    public boolean contains(SqlTypeName type) {
        return getTypeNames().contains(type);
    }
