...
public class SqlCallBinding extends SqlOperatorBinding {
  ...
  private static final SqlVisitor<Boolean> CONST_CHECKER = new ConstExpressionSqlVisitor();
  //~ Instance fields --------------------------------------------------------
  ...
  @Override public boolean isOperandParameterized(int ordinal) {

    return call.operand(ordinal) instanceof SqlDynamicParam;
  }

  @Override public boolean isConstant(int ordinal) {
    SqlNode node = call.operand(ordinal);
    if (node == null) {
      return false;
    } else {
      return node.accept(CONST_CHECKER);
    }
  }

  /**
   * A stateless visitor.
   * Check if all nodes of expression are constant value.
   */
  private static class ConstExpressionSqlVisitor implements SqlVisitor<Boolean> {
    ...
    public Boolean visit(SqlCall call) {
      boolean result = true;
      for (SqlNode operand : call.getOperandList()) {
        // true if all the operand are constant value.
        if (operand == null) {
          continue;
        }
    ...
    public Boolean visit(SqlNodeList nodeList) {
      ...
      for (int i = 0; i < nodeList.size(); i++) {
        // true if all the node are constant value.
    ...
    public Boolean visit(SqlIdentifier id) {
      // It may be a field in table.
      return false;
    }

    @Override
    public Boolean visit(SqlDataTypeSpec type) {
      // It may be a type spec in cast or convert function.
      return true;
    }
