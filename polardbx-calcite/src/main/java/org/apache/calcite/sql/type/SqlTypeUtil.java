...
public abstract class SqlTypeUtil {
    ...
    public static boolean isDateOrDatetime(RelDataType type) {
        return SqlTypeFamily.DATETIME.contains(type) && !isTime(type);
    }
    ...
    public static boolean isBigintUnsignedTypes(RelDataType type) {
        SqlTypeName typeName = type.getSqlTypeName();
        if (typeName == null) {
            return false;
        }
        return SqlTypeName.BIGINT_UNSIGNED_TYPES.contains(typeName);
    }
    ...
    public static boolean isCharTypes(RelDataType type) {
        ...
        return SqlTypeName.CHAR_TYPES.contains(typeName);
    }
    ...
    public static boolean isDatetimeTypes(RelDataType type) {
        ...
        return SqlTypeName.DATETIME_TYPES.contains(typeName);
    }
    ...
    public static boolean isDatetimeYearTypes(RelDataType type) {
        ...
        return SqlTypeName.DATETIME_YEAR_TYPES.contains(typeName);
    }
     ...
     * @return true if type is in SqlTypeFamily.Character
     */
    public static boolean inDateTimeFamily(RelDataType type) {
        return SqlTypeFamily.DATETIME.contains(type);
    }
    ...
    /**
     * using java primitive type to implement vectorization.
     */
    public static boolean isJavaPrimitiveType(RelDataType type) {
        SqlTypeName typeName = type.getSqlTypeName();
        if (typeName == null) {
            return false;
        }
     ...
     * @return true if type is one of the integer types
     */
    public static boolean isUnderLongType(RelDataType type) {
        SqlTypeName typeName = type.getSqlTypeName();
        if (typeName == null) {
            return false;
        }
    ...
    public static boolean isExactNumeric(RelDataType type) {
        ...
        case MEDIUMINT:
        case MEDIUMINT_UNSIGNED:
        case BOOLEAN: //MySQL compliance, regard boolean as tinyint.
        case YEAR: // yes, YEAR belongs to ExactNumeric family..
            return true;
        default:
            return false;
        }
     ...
     * @return true if type is numeric with exact precision
     */
    public static int exactNumberRestrcitiveLevel(RelDataType type) {
        SqlTypeName typeName = type.getSqlTypeName();
        switch (typeName) {
        //MySQL compliance, regard boolean as tinyint.
        case TINYINT:
        case BOOLEAN:
            return 1;
        case TINYINT_UNSIGNED:
        case YEAR:
            // yes, YEAR belongs to ExactNumeric family..;
            return 2;
        case SMALLINT:
            return 3;
        case SMALLINT_UNSIGNED:
    ...
    public static RelDataType addCharsetAndCollation(
        ...
        if (collation == null) {
            // this collation might be modified somewhere else
    ...
    public static boolean isEnum(RelDataType type) {
        return type.getSqlTypeName() == SqlTypeName.ENUM;
    }

    public static boolean isFractionalTypes(RelDataType type) {
        return FRACTIONAL_TYPES.contains(type.getSqlTypeName());
    }

    public static boolean isIntTypeAndBoolean(RelDataType type) {
        if (isIntType(type)) {
            return true;
        }
    ...
    public static boolean isUnsigned(RelDataType type) {
        SqlTypeName typeName = type.getSqlTypeName();
        if (typeName == null) {
            return false;
        }
        return SqlTypeName.UNSIGNED_TO_SIGNED.containsKey(typeName);
    }
}
