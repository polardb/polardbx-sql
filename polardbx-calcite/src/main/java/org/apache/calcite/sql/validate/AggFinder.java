...
class AggFinder extends SqlBasicVisitor<Void> {
  ...
  /** if not null, find the agg contain id */
  ...
  public Void visit(SqlCall call) {
    ...
    if (operator.isAggregator()
      ...
      if (aggregate) {
        if (id != null) {
          AggIdentifierFinder aggIdentifierFinder = new AggIdentifierFinder(id);
          if (aggIdentifierFinder.find(call)) {
  ...
  public void setId(SqlIdentifier id) {
    this.id = id;
  }

  private class AggIdentifierFinder extends SqlShuttle {

    public SqlIdentifier id;

    private boolean found = false;

    public AggIdentifierFinder(SqlIdentifier id) {
      this.id = id;
    }

    public boolean find(SqlCall sqlCall) {
      sqlCall.accept(this);
      return found;
    }
    ...
    public SqlNode visit(SqlIdentifier id) {
      // compare address instead of equal method
      if (this.id == id) {
        found = true;
      }
      return id;
