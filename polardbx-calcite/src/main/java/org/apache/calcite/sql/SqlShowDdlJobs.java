...
/**
 * @version 1.0
 * @ClassName SqlShowDdlJobs
 * @description
 * @Author zzy
 * @Date 2019-06-24 10:12
 */
public class SqlShowDdlJobs extends SqlShow {
    ...
    public SqlShowDdlJobs(SqlParserPos pos, List<SqlSpecialIdentifier> specialIdentifiers, boolean full, List<Long> jobIds){
    ...
    public SqlOperator getOperator() {
        if (null == operator) {
            operator = new SqlShowDdlJobsOperator(this.full);
        }
        return operator;
    }
    ...
    public SqlKind getShowKind() {
        return SqlKind.SHOW_DDL_JOBS;
    }

    public static class SqlShowDdlJobsOperator extends SqlSpecialOperator {

        private boolean full;

        public SqlShowDdlJobsOperator(boolean full){
            super("SHOW_DDL_JOBS", SqlKind.SHOW_DDL_JOBS);
            this.full = full;
        }
        ...
        public RelDataType deriveType(SqlValidator validator, SqlValidatorScope scope, SqlCall call) {
            ...
            if (full) {
                columns.add(new RelDataTypeFieldImpl("PARENT_JOB_ID", 1, typeFactory.createSqlType(SqlTypeName.INTEGER)));
            }
            ...
            if (full) {
                columns.add(new RelDataTypeFieldImpl("NEW_OBJECT_NAME", 5, typeFactory.createSqlType(SqlTypeName.VARCHAR)));
            }
            ...
            columns.add(new RelDataTypeFieldImpl("BACKFILL_PROGRESS", 14, typeFactory.createSqlType(SqlTypeName.VARCHAR)));
