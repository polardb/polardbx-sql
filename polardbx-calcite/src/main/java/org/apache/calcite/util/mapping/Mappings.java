...
public abstract class Mappings {
  ...
  public static List<Integer> identityMapping(int size) {
  ...
  private static class PartialFunctionImpl extends AbstractMapping
    ...
    public int getTargetOpt(int source) {
      if (source >= targets.length || source <= -1) {
        throw new ArrayIndexOutOfBoundsException("targets.length=" + targets.length  + ",source=" + source); 
      }
      return targets[source];
    }
  }
