...
public class NlsString implements Comparable<NlsString>, Cloneable {
  ...
  public NlsString(
    ...
    if (null != charsetName) {
      ...
      try {
        // use charset.encode instead of charset.encoder.encode,
        // because of the former always replaces malformed-input and unmappable-character
        this.charset.encode(CharBuffer.wrap(value));
      } catch (Error ex) {
