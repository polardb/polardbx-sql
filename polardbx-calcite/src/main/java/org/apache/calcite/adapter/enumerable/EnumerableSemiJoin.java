...
public class EnumerableSemiJoin extends SemiJoin implements EnumerableRel {
  ...
  EnumerableSemiJoin(
    ...
    super(cluster, traits, left, right, condition, leftKeys, rightKeys, hints);
  }
  ...
  @Override public SemiJoin copy(RelTraitSet traitSet, RexNode condition,
    ...
    try {
      return new EnumerableSemiJoin(getCluster(), traitSet, left, right,
          condition, joinInfo.leftKeys, joinInfo.rightKeys, this.hints);
    } catch (InvalidRelException e) {
      // Semantic error not possible. Must be a bug. Convert to
      // internal error.
      throw new AssertionError(e);
  ...
  @Override public RelOptCost computeSelfCost(RelOptPlanner planner,
    ...
    return planner.getCostFactory().makeCost(rowCount, 0, 0, 0,0).multiplyBy(.01d);
  }
