...
public class RelMdColumnOrigins
  ...
  public Set<RelColumnOrigin> getColumnOrigins(Aggregate rel,
    ...
    if (iOutputColumn < rel.getGroupCount()) {
      ...
      return mq.getColumnOrigins(rel.getInput(), rel.getGroupSet().nth(iOutputColumn));
    }
  ...
  public Set<RelColumnOrigin> getColumnOrigins(Project rel,
    ...
    if (mq.isIgnoreProjectDeriveOriginColumn()) {
      return null;
    }
  ...
  public Set<RelColumnOrigin> getColumnOrigins(Correlate rel,
                                               RelMetadataQuery mq, int iOutputColumn) {
    if (iOutputColumn >= rel.getRowType().getFieldCount() - 1) {
      if (!rel.getJoinType().returnsJustFirstInput()) {
        // scalar subquery
        return mq.getColumnOrigins(rel.getRight(), iOutputColumn - rel.getLeft().getRowType().getFieldCount());
      }
      return null;
    }
