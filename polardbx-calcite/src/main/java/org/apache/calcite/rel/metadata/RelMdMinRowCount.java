...
public class RelMdMinRowCount
  ...
  public Double getMinRowCount(GroupJoin rel, RelMetadataQuery mq) {
    if (rel.getGroupSet().isEmpty()) {
      // Aggregate with no GROUP BY always returns 1 row (even on empty table).
      return 1D;
    }
    final Double rowCount = mq.getMinRowCount(rel.copyAsJoin(rel.getTraitSet(), rel.getCondition()));
    if (rowCount != null && rowCount >= 1D) {
      return (double) rel.getGroupSets().size();
    }
    return 0D;
  ...
  public Double getMinRowCount(Correlate rel, RelMetadataQuery mq) {
    return 0D;
  }
  ...
  public Double getMinRowCount(DynamicValues values, RelMetadataQuery mq) {
    // For Values, the minimum row count is the actual row count.
    return (double) values.getTuples().size();
  }
