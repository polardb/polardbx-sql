...
public class LoptMultiJoin {
  ...
  /**
   * map a RelNode to the total cost of the subtree tree rooted at RelNode
   */
  Map<Integer, RelOptCost> costMap;
  //~ Constructors -----------------------------------------------------------
  ...
  /**
   * @param id root of the subtree
   * @return the accumulative cost
   */
  public RelOptCost getMapCost(int id) {
    return costMap.get(id);
  }

  /**
   * add the map to mapcost
   */
  public void putMapCost(int id, RelOptCost cost) {
    costMap.put(id, cost);
  }
   ...
   * remove a node
   */
  public void removeMapCost(int id) {
    costMap.remove(id);
  }
  /**
   * clean the cost map
   */
  public void cleanMapCost() {
    costMap.clear();
  }

  public void prepareMapCost(
      RelMetadataQuery mq,
      LoptSemiJoinOptimizer semiJoinOpt) {
    //prepare the base cost
    for (int fac = 0; fac < getNumJoinFactors(); fac++) {
      putMapCost(semiJoinOpt.getChosenSemiJoin(fac).getId(),
          mq.getCumulativeCost(semiJoinOpt.getChosenSemiJoin(fac)));
    }
  }
  /**
  ...
  public void resetJoinFilters(List<RexNode> joinFilters) {
