...
public class SemiJoinJoinTransposeRule extends RelOptRule {
  ...
  public void onMatch(RelOptRuleCall call) {
    LogicalSemiJoin semiJoin = call.rel(0);
    final Join join = call.rel(1);
    if (join instanceof SemiJoin) {
      return;
    }
    if (semiJoin.getJoinType() != JoinRelType.SEMI && semiJoin.getJoinType() !=  JoinRelType.ANTI) {
      return;
    }
    ...
    List<RexNode> newInnerOperands = new ArrayList<>();
    ...
    if (nKeysFromX > 0) {
      ...
      newSemiJoinFilter = semiJoin.getCondition().accept(rexInputConverter);
      for (RexNode rexNode : semiJoin.getOperands()) {
    ...
    SemiJoin newSemiJoin = semiJoin.copy(
