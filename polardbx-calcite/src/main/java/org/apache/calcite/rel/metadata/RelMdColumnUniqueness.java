...
public class RelMdColumnUniqueness
  ...
  public Boolean areColumnsUnique(GroupJoin rel, RelMetadataQuery mq,
                                  ImmutableBitSet columns, boolean ignoreNulls) {
    // group by keys form a unique key
    ImmutableBitSet groupKey = ImmutableBitSet.range(rel.getGroupSet().cardinality());
    return columns.contains(groupKey);
  }
