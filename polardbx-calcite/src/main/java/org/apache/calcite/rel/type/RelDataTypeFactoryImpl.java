...
public abstract class RelDataTypeFactoryImpl implements RelDataTypeFactory {
  ...
  /**
   * Global cache for Key to RelDataType. Uses weak values to allow GC.
   */
  private static final LoadingCache<Key, RelDataType> KEY2TYPE_CACHE =
      CacheBuilder.newBuilder()
          .weakValues()
          .build(new CacheLoader<Key, RelDataType>() {
                @Override public RelDataType load(@Nonnull Key key) {
  ...
  @Override public RelDataType leastRestrictiveForDML(List<RelDataType> types) {
    RelDataType type = leastRestrictive(types);
    if (type != null) {
      return type;
    }
    ...
    for (int i = 1; i < types.size(); i++) {
      if (!SqlTypeUtil.isComparable(types.get(i - 1), types.get(i))) {
        isComparable = false;
        break;
      }
    }
    ...
    if (isComparable) {
      RelDataType resultType = types.get(types.size() - 1);
      RelDataType originalType = types.get(0);
      return createSqlType(resultType.getSqlTypeName(), originalType.getPrecision());
    }
  ...
  protected RelDataType leastRestrictiveStructuredType(
    ...
    for (int j = 0; j < fieldCount; ++j) {
      ...
      final int k = j;
      builder.add(
          type0.getFieldList().get(j).getName(),type0.getFieldList().get(j).getType());
//          leastRestrictiveForDML(
//              new AbstractList<RelDataType>() {
...
//              }));
  ...
  protected RelDataType canonize(final StructKind kind,
      ...
      final List<RelDataType> types) {
    final RelDataType type = KEY2TYPE_CACHE.getIfPresent(new Key(kind, names, types));
    if (type != null) {
      return type;
    }
    ...
    return KEY2TYPE_CACHE.getUnchecked(new Key(kind, names2, types2));
  }
