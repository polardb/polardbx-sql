...
public class RelMdDistinctRowCount
  ...
  public Double getDistinctRowCount(Correlate rel, RelMetadataQuery mq,
                                    ImmutableBitSet groupKey, RexNode predicate) {
    if (predicate == null || predicate.isAlwaysTrue()) {
      if (groupKey.isEmpty()) {
        return 1D;
    ...
    return mq.getDistinctRowCount(rel.getLeft(), groupKey.clear(rel.getRowType().getFieldCount()-1), predicate);
  }
