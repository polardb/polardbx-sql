...
public class RelMdUtil {
    ...
    public static RexNode makeSemiJoinSelectivityRexNode(RelMetadataQuery mq,
        ...
        final double nonEquiSelectivity = RelMdUtil.guessSelectivity(joinInfo.getRemaining(rexBuilder));
        // prefer anti join
        final double semiJoinSelectivity = equiSelectivity * nonEquiSelectivity * 1.05;
        final double selectivity;
        if (rel.getJoinType() == JoinRelType.ANTI) {
            final double antiJoinSelectivity = 1 - semiJoinSelectivity;
            if (antiJoinSelectivity <= 0) {
                selectivity = 0.1;
            } else {
                selectivity = antiJoinSelectivity;
            }
        } else {
            selectivity = Math.min(semiJoinSelectivity, 1);
        }
    ...
    public static double guessSelectivity(
        ...
        for (RexNode pred : RelOptUtil.conjunctions(predicate)) {
            if (pred.getKind() == SqlKind.IS_NOT_NULL) {
                ...
                && (((RexCall) pred).getOperator() instanceof SqlRuntimeFilterFunction)) {
                double gusse = ((SqlRuntimeFilterFunction) ((RexCall) pred).getOperator()).getGuessSelectivity();
                sel *= gusse;
    ...
    public static void setAggChildKeys(
        ...
        for (int bit : groupKey) {
            ...
            if (bit < groupCount) {
                ...
                // aggregate column -- set a bit for each argument being
                // aggregated
                AggregateCall agg = aggCalls.get(bit - (groupCount + aggRel.getIndicatorCount()));
                for (Integer arg : agg.getArgList()) {
                    childKey.set(arg);
                }
            }
    ...
    public static Double getJoinRowCount(RelMetadataQuery mq, Join join,
        ...
        double finalRowCount = product * mq.getSelectivity(join, condition);
        if (finalRowCount < left && join.getJoinType() == JoinRelType.LEFT) {
            finalRowCount = left;
        } else if (finalRowCount < right && join.getJoinType() == JoinRelType.RIGHT) {
            finalRowCount = right;
        }
    ...
    public static boolean checkInputForCollationAndLimit(RelMetadataQuery mq,
                                                         RelNode input, RelCollation collation, RexNode offset,
                                                         RexNode fetch) {
    ...
    private static double getTimeUnitCard(TimeUnit timeUnit) {
        switch (timeUnit) {
        case MONTH:
            return 12;
        case DAY:
        ...
        case DOW:
            return 7;
        case YEAR:
            return 10; // just guessed
        default:
            return 1E9;
        }
    }

    private static double getTimeUnitRangeCard(TimeUnitRange timeUnitRange) {
        if (timeUnitRange.endUnit == null) {
            return getTimeUnitCard(timeUnitRange.startUnit);
        }
        switch (timeUnitRange) {
        case YEAR_MONTH:
            return getTimeUnitCard(TimeUnit.YEAR) * getTimeUnitCard(TimeUnit.MONTH);
        case DAY_HOUR:
            return getTimeUnitCard(TimeUnit.DAY) * getTimeUnitCard(TimeUnit.HOUR);
        case DAY_MINUTE:
            return getTimeUnitRangeCard(TimeUnitRange.DAY_HOUR) * getTimeUnitCard(TimeUnit.MINUTE);
        case DAY_SECOND:
            return getTimeUnitRangeCard(TimeUnitRange.DAY_SECOND) * getTimeUnitCard(TimeUnit.SECOND);
        case DAY_MICROSECOND:
            return getTimeUnitRangeCard(TimeUnitRange.DAY_SECOND) * getTimeUnitCard(TimeUnit.MICROSECOND);
        case HOUR_MINUTE:
            return getTimeUnitCard(TimeUnit.HOUR) * getTimeUnitCard(TimeUnit.MINUTE);
        case HOUR_SECOND:
            return getTimeUnitRangeCard(TimeUnitRange.HOUR_MINUTE) * getTimeUnitCard(TimeUnit.SECOND);
        case HOUR_MICROSECOND:
            return getTimeUnitRangeCard(TimeUnitRange.HOUR_SECOND) * getTimeUnitCard(TimeUnit.MICROSECOND);
        case MINUTE_SECOND:
            return getTimeUnitCard(TimeUnit.MINUTE) * getTimeUnitCard(TimeUnit.SECOND);
        case SECOND_MICROSECOND:
            return getTimeUnitCard(TimeUnit.SECOND) * getTimeUnitCard(TimeUnit.MICROSECOND);
        default:
            return 1E9;
        }
    }

    private static double getTimeFunctionCard(SqlFunction function) {
        ...
        case "HOUR":
        case "MINUTE":
            return getTimeUnitCard(TimeUnit.valueOf(function.getName()));
        case "YEARWEEK":
            return getTimeUnitCard(TimeUnit.YEAR) * getTimeUnitCard(TimeUnit.WEEK);
        case "WEEKDAY":
        case "DAYOFWEEK":
            return getTimeUnitCard(TimeUnit.DOW);
        case "WEEKOFYEAR":
            return getTimeUnitCard(TimeUnit.WEEK);
        case "MONTHNAME":
            return getTimeUnitCard(TimeUnit.MONTH);
        case "DAYOFYEAR":
            return getTimeUnitCard(TimeUnit.DOY);
        default:
            return 1E9;
        }
    }
    ...
    private static class CardOfProjExpr extends RexVisitorImpl<Double> {
        ...
        public Double visitCall(RexCall call) {
            ...
            if (call.isA(SqlKind.MINUS_PREFIX)) {
                ...
                Comparable timeUnit = ((RexLiteral) call.operands.get(0)).getValue();
                if (timeUnit instanceof TimeUnitRange) {
                    double card = getTimeUnitRangeCard((TimeUnitRange) timeUnit);
                    distinctRowCount = Math.min(card, rowCount / 10);
                } else {
                    distinctRowCount = rowCount / 10;
                }
            } else if (call.isA(SqlKind.OTHER_FUNCTION) && (call.op instanceof SqlFunction)) {
                double card = getTimeFunctionCard((SqlFunction) call.op);
                distinctRowCount = Math.min(card, rowCount / 10);
            } else {
