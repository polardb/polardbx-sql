...
public class AggregateProjectMergeRule extends RelOptRule {
  ...
  public static RelNode apply(RelOptRuleCall call, Aggregate aggregate,
    ...
    for (AggregateCall aggregateCall : aggregate.getAggCallList()) {
      ...
      // [GROUP_CONCAT]
      if (aggregateCall instanceof GroupConcatAggregateCall && ((GroupConcatAggregateCall)aggregateCall).getOrderList() != null) {
        final ImmutableList.Builder<Integer> newOrderArgs = ImmutableList.builder();
        for (int arg : ((GroupConcatAggregateCall)aggregateCall).getOrderList()) {
          final RexNode rex = project.getProjects().get(arg);
          if (rex instanceof RexInputRef) {
            newOrderArgs.add(((RexInputRef) rex).getIndex());
          } else {
            // Cannot handle "AGG(expression)"
            return null;
          }
        }
        aggCalls.add(((GroupConcatAggregateCall)aggregateCall).copy(newArgs.build(), newFilterArg, newOrderArgs.build()));
      } else {
