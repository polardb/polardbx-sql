...
public class JoinToMultiJoinRule extends RelOptRule {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    if (origJoin.getJoinType() == JoinRelType.SEMI|| origJoin.getJoinType() == JoinRelType.ANTI) {
      return;
    }
   ...
   */
  public List<RelNode> combineInputs(
      Join join,
      RelNode left,
      RelNode right,
   ...
   */
  public List<RexNode> combineJoinFilters(
      Join joinRel,
      RelNode left,
      RelNode right) {
    JoinRelType joinType = joinRel.getJoinType();
   ...
   */
  public boolean canCombine(RelNode input, boolean nullGenerating) {
   ...
   */
  public RexNode shiftRightFilter(
      Join joinRel,
      RelNode left,
      MultiJoin right,
      RexNode rightFilter) {
   ...
   */
  public ImmutableMap<Integer, ImmutableIntList> addOnJoinFieldRefCounts(
   ...
   */
  public List<RexNode> combinePostJoinFilters(
      Join joinRel,
      RelNode left,
      RelNode right) {
    final List<RexNode> filters = Lists.newArrayList();
