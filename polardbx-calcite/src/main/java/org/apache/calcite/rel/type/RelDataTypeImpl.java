...
public abstract class RelDataTypeImpl
  ...
  public static RelProtoDataType proto(final SqlTypeName typeName, final CharsetName charsetName, final CollationName collationName, final boolean nullable) {
    return new RelProtoDataType() {
      public RelDataType apply(RelDataTypeFactory typeFactory) {
        Preconditions.checkNotNull(charsetName);
        final RelDataType type =
            typeFactory.createSqlType(typeName);
        Charset charset = Optional.of(charsetName.getJavaCharset()).map(Charset::forName).get();
        String collation = Optional.ofNullable(collationName).map(Enum::name).orElse(null);
        typeFactory.createTypeWithNullability(type, nullable);
        return Optional.of(type)
            .map(t -> typeFactory.createTypeWithNullability(t, nullable))
            .map(t -> typeFactory.createTypeWithCharsetAndCollation(t, charset, new SqlCollation(charset, collation, SqlCollation.Coercibility.IMPLICIT)))
            .get();
      }
    };
  }
