...
public class RelMdSize implements MetadataHandler<BuiltInMetadata.Size> {
  ...
  public List<Double> averageColumnSizes(Correlate rel, RelMetadataQuery mq) {
    return mq.getAverageColumnSizes(rel.getLeft());
  }
  ...
  public List<Double> averageColumnSizes(GroupJoin rel, RelMetadataQuery mq) {
    final List<Double> inputColumnSizes =
        mq.getAverageColumnSizesNotNull(rel.copyAsJoin(rel.getTraitSet(), rel.getCondition()));
    final ImmutableList.Builder<Double> list = ImmutableList.builder();
    for (int key : rel.getGroupSet()) {
      list.add(inputColumnSizes.get(key));
    }
