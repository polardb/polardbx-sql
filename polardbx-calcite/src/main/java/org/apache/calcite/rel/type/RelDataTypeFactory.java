...
public interface RelDataTypeFactory {
  ...
  RelDataType arithmeticCastLeastRestrictive(List<RelDataType> types);

    /**
   * For DML, convert value type to comparable type.
   *
   * @param types input types to be combined using union (not null, not empty)
   * @return the comparable type
   */
  RelDataType leastRestrictiveForDML(List<RelDataType> types);
