...
public class RelMdMaxRowCount
  ...
  public Double getMaxRowCount(Correlate rel, RelMetadataQuery mq) {
    return mq.getMaxRowCount(rel.getLeft());
  }
  ...
  public Double getMaxRowCount(GroupJoin rel, RelMetadataQuery mq) {
    if (rel.getGroupSet().isEmpty()) {
      // Aggregate with no GROUP BY always returns 1 row (even on empty table).
      return 1D;
    }
    final Double rowCount = mq.getMaxRowCount(rel.copyAsJoin(rel.getTraitSet(), rel.getCondition()));
    if (rowCount == null) {
      return null;
    }
    return rowCount * rel.getGroupSets().size();
  ...
  public Double getMaxRowCount(DynamicValues values, RelMetadataQuery mq) {
    // For Values, the maximum row count is the actual row count.
    // This is especially useful if Values is empty.
    return (double) values.getTuples().size();
  }
  ...
  public Double getMaxRowCount(Window rel, RelMetadataQuery mq) {
    return mq.getMaxRowCount(rel.getInput());
  }
}
