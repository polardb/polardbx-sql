...
public class BooleanBlockTest extends BaseBlockTest {
    ...
    public void test() {
        ...
        BooleanBlockBuilder blockBuilder = new BooleanBlockBuilder(CHUNK_SIZE);
        ...
        for (int i = 0; i < values.length; i++) {
            if (values[i] != null) {
                assertEquals(values[i], blockBuilder.getBoolean(i));
            } else {
        ...
        for (int i = 0; i < values.length; i++) {
            if (values[i] != null) {
                ...
                assertEquals(values[i], block.getBoolean(i));
        ...
        BlockBuilder anotherBuilder = new BooleanBlockBuilder(CHUNK_SIZE);

        for (int i = 0; i < values.length; i++) {
            block.writePositionTo(i, anotherBuilder);

            if (values[i] != null) {
                assertEquals(values[i], anotherBuilder.getBoolean(i));
            } else {
