...
public class YearColumnReaderTest extends FullTypeScanTestBase {
    ...
    protected void check(Block block, VectorizedRowBatch batch, int targetColumnId) {
        Assert.assertTrue(block instanceof LongBlock);
        ...
        for (int row = 0; row < batch.size; row++) {
            ...
            if (vector.isNull[row]) {
                ...
                Assert.assertEquals(array[idx], block.getLong(row));
            }
        }
    }
}
