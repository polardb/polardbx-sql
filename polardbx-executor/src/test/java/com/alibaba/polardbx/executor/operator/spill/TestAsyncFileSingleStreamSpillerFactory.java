...
public class TestAsyncFileSingleStreamSpillerFactory {
        ...
        for (int i = 0; i < 10; ++i) {
            SingleStreamSpiller singleStreamSpiller =
                spillerFactory.create("testDistributesSpillOverPaths", ImmutableList.of(DataTypes.LongType),
                    new QuerySpillSpaceMonitor("test").newLocalSpillMonitor(), null);
    ...
    public void testCleanupOldSpillFiles() throws Exception {
        ...
        spillPath1.mkdirs();
        spillPath2.mkdirs();
        java.nio.file.Files.createTempFile(spillPath1.toPath(), SingleStreamSpillerFactory.SPILL_FILE_PREFIX,
            SingleStreamSpillerFactory.SPILL_FILE_SUFFIX);
        java.nio.file.Files.createTempFile(spillPath1.toPath(), SingleStreamSpillerFactory.SPILL_FILE_PREFIX,
            SingleStreamSpillerFactory.SPILL_FILE_SUFFIX);
        java.nio.file.Files
            .createTempFile(spillPath1.toPath(), SingleStreamSpillerFactory.SPILL_FILE_PREFIX, "blah");
        java.nio.file.Files.createTempFile(spillPath2.toPath(), SingleStreamSpillerFactory.SPILL_FILE_PREFIX,
            SingleStreamSpillerFactory.SPILL_FILE_SUFFIX);
        java.nio.file.Files
            .createTempFile(spillPath2.toPath(), "blah", SingleStreamSpillerFactory.SPILL_FILE_SUFFIX);
        java.nio.file.Files.createTempFile(spillPath2.toPath(), "blah", "blah");

        System.out.println(spillPath1.getAbsolutePath());
        System.out.println(spillPath2.getAbsolutePath());
        System.out.println(FileUtils.listFiles(spillPath1, null, false));
        assertEquals(FileUtils.listFiles(spillPath1, null, false).size(), 3);
        assertEquals(FileUtils.listFiles(spillPath2, null, false).size(), 3);
    ...
    public void testMaxFdLimited() throws Exception {
        ...
        LocalSpillMonitor localSpillMonitor = new QuerySpillSpaceMonitor("test").newLocalSpillMonitor();
