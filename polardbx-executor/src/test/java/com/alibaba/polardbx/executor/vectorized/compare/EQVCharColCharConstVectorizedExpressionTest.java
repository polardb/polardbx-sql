...
public class EQVCharColCharConstVectorizedExpressionTest {
    ...
    public EQVCharColCharConstVectorizedExpressionTest(boolean compatible) {
    ...
    public void testSlice() {
        SliceBlockBuilder blockBuilder = new SliceBlockBuilder(DataTypes.CharType, count, context, enableCompatible);
        ...
        LongBlock expectBlock = LongBlock.of(0L, 1L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, null);
    ...
    public void testDict() {
        ...
        blockBuilder.appendNull();
        SliceBlock sliceBlock = (SliceBlock) blockBuilder.build();
        sliceBlock = getBlockWithDict(sliceBlock);
    ...
    @Test
    public void testDictWithSelection() {
        SliceBlockBuilder blockBuilder = new SliceBlockBuilder(DataTypes.CharType, count, context, enableCompatible);
        for (int i = 0; i < count - 1; i++) {
            blockBuilder.writeString(String.valueOf(i * 100));
        }
    ...
    public void testDictNotMatch() {
        ...
        LongBlock expectBlock = LongBlock.of(0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, null);

        doTest(inputChunk, expectBlock, "999", null);
    }

    @Test
    public void testDictWithSelectionNotMatch() {
        SliceBlockBuilder blockBuilder = new SliceBlockBuilder(DataTypes.CharType, count, context, enableCompatible);
        for (int i = 0; i < count - 1; i++) {
            blockBuilder.writeString(String.valueOf(i * 100));
        }
        ...
        LongBlock expectBlock = LongBlock.of(0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, null);
        int[] sel = new int[] {0, 1, 2, 4, 5};
        doTest(inputChunk, expectBlock, "999", sel);
    }
    ...
    private void doTest(Chunk inputChunk, RandomAccessBlock expectBlock, String value, int[] sel) {
        ...
        Assert.assertTrue(condition instanceof EQCharColCharConstVectorizedExpression);
    ...
    private void doTestNull(Chunk inputChunk, RandomAccessBlock expectBlock, int[] sel) {
        EQCharColCharConstVectorizedExpression condition = new EQCharColCharConstVectorizedExpression(
            2,
            new VectorizedExpression[] {
                new InputRefVectorizedExpression(DataTypes.CharType, 0, 0),
                new LiteralVectorizedExpression(DataTypes.CharType, null, 1)
            });

        // placeholder for input and output blocks
        MutableChunk preAllocatedChunk = MutableChunk.newBuilder(context.getExecutorChunkLimit())
            .addEmptySlots(Collections.singletonList(DataTypes.CharType))
    ...
    private SliceBlock getBlockWithDict(SliceBlock sliceBlock) {
        int count = sliceBlock.getPositionCount();
        Slice[] slices = new Slice[sliceBlock.getPositionCount()];
        int[] dictIds = new int[sliceBlock.getPositionCount()];
        for (int i = 0; i < slices.length; i++) {
            slices[i] = sliceBlock.getRegion(i);
            dictIds[i] = i;
        }
        BlockDictionary dictionary = new LocalBlockDictionary(slices);
        return new SliceBlock((SliceType) sliceBlock.getType(), 0, count, sliceBlock.nulls(),
            dictionary, dictIds, true);
    }
}
