...
public class ColumnarManagerTest {
    private static final List<MockFileStatus> MOCK_FILE_STATUSES = new ArrayList<>(Arrays.asList(
        new MockFileStatus("1.csv", "p1", 1, 2),
        new MockFileStatus("1.orc", "p1", 1, 2),
        new MockFileStatus("1.del", "p1", 1, 2),
        new MockFileStatus("2.csv", "p1", 2, 4),
        new MockFileStatus("2.orc", "p1", 2, 4),
        new MockFileStatus("2.del", "p1", 2, 4),
        new MockFileStatus("3.csv", "p1", 3, 5),
        new MockFileStatus("3.orc", "p1", 3, 5),
        new MockFileStatus("3.del", "p1", 3, 5),
        new MockFileStatus("4.csv", "p2", 1, 5),
        new MockFileStatus("4.orc", "p2", 1, 5),
        new MockFileStatus("4.del", "p2", 1, 5)
    ));

    private static final List<MockFileStatus> MOCK_COMPACTION_STATUSES = Arrays.asList(
        new MockFileStatus("5.csv", "p2", 2, 5),
        new MockFileStatus("5.orc", "p2", 2, 5),
        new MockFileStatus("5.del", "p2", 2, 5)
    );
    ...
    public void prepareFileVersionStorage() {
        try {
            ...
            mockFilesAccessorCtor = Mockito.mockConstruction(FilesAccessor.class, (mock, context) -> {
                Mockito.when(mock.queryColumnarDeltaFilesByTsoAndTableId(Mockito.anyLong(), Mockito.anyLong(),
    ...
    public void testPurgeWithMultiVersionSchema() {
        ...
        try (MockedStatic<ColumnarTransactionUtils> columnarTransactionUtilsMockedStatic = Mockito.mockStatic(
            ...
            cm.injectForTest(mockFileVersionStorage, mockMultiVersionColumnarSchema);
            Assert.assertEquals(minTso, cm.getMinTso().longValue());
            Assert.assertEquals(initTso, cm.latestTso());
    ...
    static class MockFileStatus {
        ...
        MockFileStatus(String fileName, String partitionName, long commitTso, long removeTso) {
