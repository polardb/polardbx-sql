...
function StatementClient(connectionData, headerCallback, dataCallback, errorCallback) {
    ...
    this.headers = {
        "X-PolarDBX-User": this.user ? this.user : 'N/A',
        "X-PolarDBX-Source": this.source,
        "X-PolarDBX-Session": this.sessionParameters
    };

    if (!(this.catalog === undefined)) {
        this.headers["X-PolarDBX-Catalog"] = this.catalog
    }

    if (!(this.schema === undefined)) {
        this.headers["X-PolarDBX-Schema"] = this.schema
    }
