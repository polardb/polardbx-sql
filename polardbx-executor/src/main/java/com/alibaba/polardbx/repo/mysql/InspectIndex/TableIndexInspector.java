...
public class TableIndexInspector {
    ...
    public static Map<String, TableIndexInspector> createTableInspectorsInSchema(String schema) {
        ...
        for (InspectIndexInfo info : allInfos) {
            ...
            //rectify gsi cardinality
            if ("key".equalsIgnoreCase(info.tbPartitionPolicy)) {
                String cardinalityColumn = info.tbPartitionColumns.get(0);
                StatisticResult cardinalityResult =
                    StatisticManager.getInstance()
                        .getCardinality(schema, tableName, cardinalityColumn, false, false);
                info.rowCardinality = cardinalityResult.getLongValue();
            }
        }
    ...
    public TableIndexInspector inspectUseFrequency() {
        boolean enableGsiStatisticCollection = GsiStatisticsManager.enableGsiStatisticsCollection();
        if (!enableGsiStatisticCollection) {
            return this;
        }
    ...
    /**
     * 定义: discrimination = distinct(gsiRow) / gsi分区个数
     * 经验值: discrimination 大于3属正常
     */
    public TableIndexInspector inspectDiscrimination() {
        final Double minDiscrimination = 3.0;
    ...
    protected void inspectDuplicateLsi() {
        ...
        for (Map.Entry<String, List<String>> entry : hashTableOnIndexColumns.entrySet()) {
            ...
            for (int i = 0; i < indexes.size(); i++) {
                ...
                if (!isAutoGeneratedLsi(indexes.get(i)) && localIndexInspectInfo.get(indexes.get(i)).unique) {
                    reserveIdx = i;
                    break;
                }
            }
    ...
    protected void inspectSubFragmentInPrimaryKeyLsi() {
        for (Map.Entry<String, InspectIndexInfo> entry : localIndexInspectInfo.entrySet()) {
            String lsiName = entry.getKey();
            if (isAutoPartitionLsi(lsiName)) {
                continue;
            ...
            InspectIndexInfo info = entry.getValue();
            if (containInPrefix(info.indexColumns, primaryColumns)) {
                info.problem.put(
    ...
    public TableIndexInspector inspectLsi() {
        ...
        inspectSubFragmentInPrimaryKeyLsi();
    ...
    public static Set<String> queryTableNamesFromSchema(String schema) {
        final String showTablesFromSql =
        ...
        for (Map<String, Object> row : result) {
            Object val = row.get("table_name");
            if (val instanceof Slice) {
                String tbName = ((Slice) val).toStringUtf8();
                tables.add(tbName);
            }
        }
        return tables;
    ...
    private static List<InspectIndexInfo> queryGsiInspectInfoRecordFromSchema(String schema) {
        ...
        for (Map<String, Object> row : result) {
            ...
            //gsi row discrimination = cardinality / partitionNum
            int partitionNum = queryPartitonNumOnGsi(schema, indexName);

            if (info.rowCardinality > 0 && partitionNum != 0) {
                info.rowDiscrimination = info.rowCardinality * 1.0 / partitionNum;
            } else {
    ...
    private static int queryPartitonNumOnGsi(String schema, String gsiName) {
        final String querySql = "show topology from " + quoteName(gsiName);
        List<Map<String, Object>> result = DdlHelper.getServerConfigManager().executeQuerySql(querySql, schema, null);
        return result.size();
    }
