...
public class DdlEngineRecoverJobsHandler extends DdlEngineContinueJobsHandler {

    public DdlEngineRecoverJobsHandler(IRepository repo) {
        super(repo);
    }

    @Override
    public Cursor doHandle(final LogicalDal logicalPlan, ExecutionContext executionContext) {
        SqlRecoverDdlJob command = (SqlRecoverDdlJob) logicalPlan.getNativeSqlNode();
