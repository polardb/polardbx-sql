...
public class FinalizerService {
    private static final Logger log = LoggerFactory.getLogger(FinalizerService.class);
    ...
    private final ExecutorService executor = newSingleThreadExecutor(Threads.daemonThreadsNamed("FinalizerService"));
    ...
    private void processFinalizerQueue() {
        while (!Thread.currentThread().isInterrupted()) {
            try {
                ...
                log.error("Finalizer cleanup failed", e);
            }
        }
    }
