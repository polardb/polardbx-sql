...
public class RenameTablePhyObjectRecorder extends GenericPhyObjectRecorder {
    ...
    public RenameTablePhyObjectRecorder(RelNode physicalPlan, ExecutionContext executionContext) {
    ...
    private void revertForRollback() {
        ...
        if (isRenameRollback) {
            ...
            Map<Integer, ParameterContext> params = physicalDdlPlan.getParam();
            if (params != null && params.size() == 2) {
                ParameterContext paramOldName = params.get(1);
                ParameterContext paramNewName = params.get(2);
                Object oldName = paramOldName.getValue();
                paramOldName.setValue(paramNewName.getValue());
                paramNewName.setValue(oldName);
            }
        }
    }
