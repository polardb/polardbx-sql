 ...
 */
@Getter
@TaskName(name = "UnlockTableSyncTask")
public class UnlockTableSyncTask extends BaseSyncTask {

    final String primaryTableName;

    @JSONCreator
    public UnlockTableSyncTask(String schemaName,
    ...
    protected void executeImpl(ExecutionContext executionContext) {
        ...
        try {
            ...
            LOGGER.info(
                String.format("finished unlock table after physical cutover for primary table: %s.%s", schemaName,
                    primaryTableName)
            );

        } catch (Exception e) {
            String errMsg = String.format(
                "error occurs while unlock table after physical cutover, tableName:%s", primaryTableName
            );
            LOGGER.error(errMsg);
