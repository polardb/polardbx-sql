 ...
 **/
public class LogicalCancelReplicaCheckTableHandler extends HandlerCommon {

    private static final String API_PATTERN = "http://%s/replica/fullValidation/cancel";

    public LogicalCancelReplicaCheckTableHandler(IRepository repo) {
        super(repo);
    }

    @Override
    public Cursor handle(RelNode logicalPlan, ExecutionContext executionContext) {
        SqlCancelReplicaCheck sqlCancelReplicaCheck =
            (SqlCancelReplicaCheck) ((LogicalDal) logicalPlan).getNativeSqlNode();

        String dbName = sqlCancelReplicaCheck.getDbName().toString();
        if (StringUtils.isEmpty(dbName)) {
            throw new TddlRuntimeException(ErrorCode.ERR_REPLICATION_RESULT, "channel and database cannot be empty!");
        }
        Map<String, String> params = new HashMap<>();
        params.put(RplConstants.RPL_FULL_VALID_DB, dbName);
        if (sqlCancelReplicaCheck.getTableName() != null) {
            String tbName = sqlCancelReplicaCheck.getTableName().toString();
