...
public class BloomFilterConsumeFilter {
    ...
    public boolean filter(Chunk.ChunkRow row) {
        // Build condition (boolean) block
        boolean bfilter = false;

        if (bloomFilterConsume.getBloomFilter() != null) {
            if (hasher == null) {
                hasher = bloomFilterConsume.getBloomFilter().newHasher();
            }
            bfilter = !bloomFilterConsume.getBloomFilter()
