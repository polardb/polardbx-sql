...
@Getter
@Setter
public class ActionSplitTablePartition implements BalanceAction {

    private final static Logger LOG = LoggerFactory.getLogger(ActionSplitTablePartition.class);
    private final static String NAME = "SplitTablePartition";
    private static final String SPLIT_RANGE_PARTITION_SQL = "alter table %s split partition %s into (%s)";
    ...
    public ActionSplitTablePartition(String schemaName, String tableName, String partitionName, String genSql) {
    ...
    public ActionSplitTablePartition(String schema, PartitionStat partition, List<SplitPoint> splitPointList,
                                     BalanceStats stats) {
        this.schema = schema;
        this.tableName = partition.getPartitionRecord().getTableName();
        ...
        if (TStringUtil.isBlank(genSql)) {
            ...
            String res =
                String.format(SPLIT_RANGE_PARTITION_SQL,
                    TStringUtil.backQuote(this.getTableName()),
    ...
    public ExecutableDdlJob toDdlJob(ExecutionContext ec) {
        ...
        List<PartitionStat> partitionStatList =
            stats.filterTablePartitionStat(tableName, Sets.newHashSet(partitionName));
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ActionSplitTablePartition)) {
            return false;
        }
        ActionSplitTablePartition that = (ActionSplitTablePartition) o;
    ...
    public Long getBackfillRows() {
        return stats.getPartitionStats().stream().filter(
                o -> o.getPartitionRecord().getTableName().equals(this.tableName) && partitionName.equals(
                    o.getPartitionName()))
    ...
    @Override
    public int hashCode() {
        return Objects.hash(tableName, partitionName, rightBound, splitPoints);
    }
