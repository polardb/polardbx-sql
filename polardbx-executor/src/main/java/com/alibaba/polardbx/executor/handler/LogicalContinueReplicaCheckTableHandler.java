 ...
 **/
public class LogicalContinueReplicaCheckTableHandler extends HandlerCommon {

    private static final String API_PATTERN = "http://%s/replica/fullValidation/continue";

    public LogicalContinueReplicaCheckTableHandler(IRepository repo) {
        super(repo);
    }

    @Override
    public Cursor handle(RelNode logicalPlan, ExecutionContext executionContext) {
        SqlContinueReplicaCheck sqlContinueReplicaCheck =
            (SqlContinueReplicaCheck) ((LogicalDal) logicalPlan).getNativeSqlNode();

        String dbName = sqlContinueReplicaCheck.getDbName().toString();
        ...
        if (sqlContinueReplicaCheck.getTableName() != null) {
            String tbName = sqlContinueReplicaCheck.getTableName().toString();
