...
@ExpressionSignatures(names = {"AND"}, argumentTypes = {"Long", "Long", "Long", "Long", "Long"}, argumentKinds = {Variable, Variable, Variable, Variable, Variable})
public class AndLongColLongColLongColLongColLongColVectorizedExpression extends AbstractVectorizedExpression {
    public AndLongColLongColLongColLongColLongColVectorizedExpression(
            int outputIndex,
    ...
    public void eval(EvaluationContext ctx) {
        ...
        RandomAccessBlock inputVec5 = chunk.slotIn(children[4].getOutputIndex(), children[4].getOutputDataType());
        ...
        long[] array5 = ((LongBlock) inputVec5).longArray();
        boolean[] nulls5 = inputVec5.nulls();
        boolean input5HasNull = inputVec5.hasNull();

        long[] res = ((LongBlock) outputVectorSlot).longArray();
        boolean[] outputNulls = outputVectorSlot.nulls();
        outputVectorSlot.setHasNull(input1HasNull | input2HasNull | input3HasNull | input4HasNull | input5HasNull);
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                ...
                boolean null5 = !input5HasNull ? false : nulls5[j];
                boolean b1 = (array1[j] != 0);
                boolean b2 = (array2[j] != 0);
                boolean b3 = (array3[j] != 0);
                boolean b4 = (array4[j] != 0);
                boolean b5 = (array5[j] != 0);

                outputNulls[j] = null1 || null2 || null3 || null4 || null5;
                res[j] = (b1 && b2 && b3 && b4 && b5) ? LongBlock.TRUE_VALUE : LongBlock.FALSE_VALUE;
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                ...
                boolean null5 = !input5HasNull ? false : nulls5[i];
                boolean b1 = (array1[i] != 0);
                boolean b2 = (array2[i] != 0);
                boolean b3 = (array3[i] != 0);
                boolean b4 = (array4[i] != 0);
                boolean b5 = (array5[i] != 0);

                outputNulls[i] = null1 || null2 || null3 || null4 || null5;
                res[i] = (b1 && b2 && b3 && b4 && b5) ? LongBlock.TRUE_VALUE : LongBlock.FALSE_VALUE;
            }
        }
    }
}
