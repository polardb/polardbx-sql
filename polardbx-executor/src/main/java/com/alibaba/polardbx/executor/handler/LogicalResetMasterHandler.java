 ...
 **/
public class LogicalResetMasterHandler extends HandlerCommon {

    private static final Logger cdcLogger = SQLRecorderLogger.cdcLogger;

    public LogicalResetMasterHandler(IRepository repo) {
        super(repo);
    }

    @Override
    public Cursor handle(RelNode logicalPlan, ExecutionContext executionContext) {
        SqlResetMaster sqlResetMaster = (SqlResetMaster) ((LogicalDal) logicalPlan).getNativeSqlNode();
        SqlNode with = sqlResetMaster.getWith();
        ...
        String res;
        try {
            res = HttpClientHelper.doGet("http://" + daemonEndpoint + "/system/reset");
        } catch (Exception e) {
            cdcLogger.error("reset master error!", e);
            throw new RuntimeException("reset master error", e);
        }
        ...
        try {
            res = HttpClientHelper.doGet("http://" + daemonEndpoint + "/system/cleanBinlog");
        } catch (Exception e) {
            cdcLogger.error("clean cdc binlog error!", e);
            throw new RuntimeException("reset master error", e);
        }
        resultCode = JSON.parseObject(res, ResultCode.class);
        if (resultCode.getCode() != CdcConstants.SUCCESS_CODE) {
            cdcLogger.warn("clean cdc binlog failed! code:" + resultCode.getCode() + ", msg:" + resultCode.getMsg());
            throw new TddlRuntimeException(ErrorCode.ERR_CDC_GENERIC, resultCode.getMsg());
        }
