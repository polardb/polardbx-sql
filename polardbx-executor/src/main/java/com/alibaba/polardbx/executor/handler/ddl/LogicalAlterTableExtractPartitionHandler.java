...
import com.alibaba.polardbx.executor.ddl.job.factory.AlterTableExtractPartitionJobFactory;
...
public class LogicalAlterTableExtractPartitionHandler extends LogicalCommonDdlHandler {

    public LogicalAlterTableExtractPartitionHandler(IRepository repo) {
        super(repo);
    }
    ...
    protected DdlJob buildDdlJob(BaseDdlOperation logicalDdlPlan, ExecutionContext executionContext) {
        ...
        alterTableExtractPartition.preparedData(executionContext);
        return AlterTableExtractPartitionJobFactory
            .create(alterTableExtractPartition.relDdl,
                (AlterTableExtractPartitionPreparedData) alterTableExtractPartition.getPreparedData(),
                executionContext);
    }

    @Override
    protected boolean validatePlan(BaseDdlOperation logicalDdlPlan, ExecutionContext executionContext) {
        LogicalAlterTableExtractPartition logicalAlterTableExtractPartition =
            (LogicalAlterTableExtractPartition) logicalDdlPlan;
        AlterTable alterTable = (AlterTable) logicalAlterTableExtractPartition.relDdl;
        String logicalTableName = Util.last(((SqlIdentifier) alterTable.getTableName()).names);
        boolean isNewPart =
            DbInfoManager.getInstance().isNewPartitionDb(logicalAlterTableExtractPartition.getSchemaName());
        if (!isNewPart) {
            throw new TddlRuntimeException(ErrorCode.ERR_GMS_GENERIC,
                "can't execute the extract partition command in drds mode");
        }
