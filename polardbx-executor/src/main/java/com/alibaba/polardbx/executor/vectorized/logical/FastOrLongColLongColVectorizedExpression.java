...
public class FastOrLongColLongColVectorizedExpression extends AbstractVectorizedExpression {

    private int[] tmpSelectionBuffer = null;

    public FastOrLongColLongColVectorizedExpression(int outputIndex, VectorizedExpression[] children) {
    ...
    public void eval(EvaluationContext ctx) {
        final boolean enableShortCircuit =
            ctx.getExecutionContext().getParamManager().getBoolean(ConnectionParams.ENABLE_OR_FAST_VEC);
        ...
        doOrEvalWithShortCircuit(ctx);
    }

    private void doOrEvalWithShortCircuit(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                // select when left result is false
        ...
        // pass tmp selection to right expression
        children[1].eval(ctx);
