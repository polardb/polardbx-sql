...
@Getter
@TaskName(name = "CleanUpdateDbLocalityMetaTask")
public class CleanUpdateDbLocalityMetaTask extends BaseDdlTask {

    private List<LocalityDetailInfoRecord> toRemovedLocalityItem;

    private List<Pair<LocalityDetailInfoRecord, LocalityDetailInfoRecord>> toUpdateLocalityItem;

    @JSONCreator
    public CleanUpdateDbLocalityMetaTask(String schemaName, List<LocalityDetailInfoRecord> toRemovedLocalityItem,
                                         List<Pair<LocalityDetailInfoRecord, LocalityDetailInfoRecord>> toUpdateLocalityItem) {
        super(schemaName);
        this.toRemovedLocalityItem = toRemovedLocalityItem;
        this.toUpdateLocalityItem = toUpdateLocalityItem;
    }
    ...
    protected void executeImpl(Connection metaDbConnection, ExecutionContext executionContext) {
        ...
        for (int i = 0; i < toRemovedLocalityItem.size(); i++) {
            ...
            if (objectType == LocalityDetailInfoRecord.LOCALITY_TYPE_TABLEGROUP) {
                ...
                tablePartitionAccessor.setTableLocalityByTableName(schemaName,
                    localityDetailInfoRecord.objectName, "");
        ...
        Map<String, String> tableNamesAndLocality = new HashMap<>();
        for (int i = 0; i < toUpdateLocalityItem.size(); i++) {
            LocalityDetailInfoRecord localityDetailInfoRecord = toUpdateLocalityItem.get(i).getValue();
            int objectType = localityDetailInfoRecord.getObjectType();
            if (objectType == LocalityDetailInfoRecord.LOCALITY_TYPE_TABLEGROUP) {
                tableGroupAccessor.updateTableGroupLocality(dbName, localityDetailInfoRecord.getObjectName(),
                    localityDetailInfoRecord.locality);
                //table partition
            } else if (objectType == LocalityDetailInfoRecord.LOCALITY_TYPE_PARTITIONGROUP) {
//                partitionGroupAccessor.updatePartitionGroupLocality(localityDetailInfoRecord.getObjectId(),
//                    localityDetailInfoRecord.locality);
//                removedLocalityPgIds.add(localityDetailInfoRecord.getObjectId());
                ...
                tableNames.add(localityDetailInfoRecord.objectName);
                tableNamesAndLocality.put(localityDetailInfoRecord.objectName, localityDetailInfoRecord.locality);
            } else if (objectType == LocalityDetailInfoRecord.LOCALITY_TYPE_DATABASE) {
                LocalityManager.getInstance().setLocalityOfDb(localityDetailInfoRecord.getObjectId(),
                    localityDetailInfoRecord.getLocality());
            }
        }
        tablePartitionAccessor.updateTableLocalityByTableNameAndLocality(schemaName, tableNamesAndLocality);
        /**
    ...
    protected void rollbackImpl(Connection metaDbConnection, ExecutionContext executionContext) {
        ...
        Map<String, String> tableNamesAndLocality = new HashMap<>();
        for (int i = 0; i < toUpdateLocalityItem.size(); i++) {
            LocalityDetailInfoRecord localityDetailInfoRecord = toUpdateLocalityItem.get(i).getKey();
            int objectType = localityDetailInfoRecord.getObjectType();
            if (objectType == LocalityDetailInfoRecord.LOCALITY_TYPE_TABLEGROUP) {
                tableGroupAccessor.updateTableGroupLocality(dbName, localityDetailInfoRecord.getObjectName(),
                    localityDetailInfoRecord.locality);
