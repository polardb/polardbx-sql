...
import com.alibaba.polardbx.optimizer.state.NullableIntegerGroupState;

public class Int2IntMax extends AbstractAggregator {

    protected NullableIntegerGroupState groupState;

    public Int2IntMax(int index, DataType inputType, DataType outputType, int filterArg) {
        super(new int[] {index}, false, new DataType[] {inputType}, outputType, filterArg);
    }

    @Override
    public void open(int capacity) {
        groupState = new NullableIntegerGroupState(capacity);
    }
    ...
    public void accumulate(int groupId, Chunk chunk, int position) {
        ...
        final int value = block.getInt(position);
        if (groupState.isNull(groupId)) {
            groupState.set(groupId, value);
        } else {
            int beforeValue = groupState.get(groupId);
            int afterValue = Math.max(value, beforeValue);
    ...
    public void writeResultTo(int groupId, BlockBuilder bb) {
        if (groupState.isNull(groupId)) {
            ...
            bb.writeInt(groupState.get(groupId));
        }
    }
