...
public class MoveTableSyncAction implements ISyncAction {

    private String schemaName;
    private String logicalTableName;

    public MoveTableSyncAction() {

    }

    public MoveTableSyncAction(String schemaName, String logicalTableName) {
    ...
    public ResultCursor sync() {
        ...
        TableMeta tableMeta = oldSchemaManager.getTable(logicalTableName);
        if (tableMeta.isGsi()) {
            //for gsi table sync it's primary table, before gsi table never get the mdl lock
            ((GmsTableMetaManager) oldSchemaManager)
                .tonewversion(tableMeta.getGsiTableMetaBean().gsiMetaBean.tableName);
        } else {
