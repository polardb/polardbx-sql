...
public class AlterTableGroupReorgPartitionBuilder extends AlterTableGroupBaseBuilder {

    public AlterTableGroupReorgPartitionBuilder(DDL ddl,
    ...
    protected void generateNewPhysicalTableNames(List<String> allLogicalTableNames) {
        AlterTableGroupReorgPartitionPreparedData preparedData =
            (AlterTableGroupReorgPartitionPreparedData) this.preparedData;
        if (preparedData.isReorgSubPartition() || preparedData.isUseTemplatePart()) {
                ...
                try (Connection conn = MetaDbDataSource.getInstance().getConnection()) {
                    ...
                    try {
                        ...
                        List<TableGroupRecord> tableGroupRecords =
                            accessor.getTableGroupsBySchemaAndName(schemaName, tableGroupName, true);
                        assert tableGroupRecords.size() == 1;
                        TableGroupRecord tableGroupRecord = tableGroupRecords.get(0);

                        int[] minPostfix = new int[1];
                        int maxPostfix = 1;
                        for (String logicalTableName : allLogicalTableNames) {
                            minPostfix[0] = tableGroupRecord.getInited() - preparedData.getNewPartitionNames().size();
                            minPostfix[0] = Math.max(minPostfix[0], 0);
                            PartitionInfo currPartitionInfo =
                                oc.getPartitionInfoManager().getPartitionInfo(logicalTableName);
                            List<String> physicalTables = PartitionInfoUtil.getNextNPhyTableNames(currPartitionInfo,
                                preparedData.getNewPartitionNames().size(), minPostfix);
                            maxPostfix = Math.max(minPostfix[0], maxPostfix);
                            newPhysicalTables.put(logicalTableName, physicalTables);
                        }
