...
@ExpressionSignatures(names = {"CastToSigned", "ConvertToSigned"}, argumentTypes = {"Decimal"},
    argumentKinds = {Variable})
public class CastDecimalToSignedVectorizedExpression extends AbstractVectorizedExpression {

    public CastDecimalToSignedVectorizedExpression(DataType<?> outputDataType, int outputIndex,
    ...
    public void eval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                ...
                long l = DecimalConverter.decimal2Long(tmpDecimal)[0];
                output[j] = l;
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
