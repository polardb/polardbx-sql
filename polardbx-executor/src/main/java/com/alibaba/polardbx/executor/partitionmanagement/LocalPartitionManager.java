...
public class LocalPartitionManager {
            ...
            for (List<String> list : phyTableList) {
                ...
                for (String phyTableName : list) {
                    List<LocalPartitionDescription> localPartitionList =
                        CheckTableUtil.getLocalPartitionDescription(repo, schemaName, groupName, phyTableName);
     ...
     */
    public static List<LocalPartitionDescription> getExpiredLocalPartitionDescriptionList(
        ...
        List<LocalPartitionDescription> candidates = new ArrayList<>(partitionList);
        candidates.sort(LocalPartitionDescription::comparePartitionOrdinalPosition);
        for (LocalPartitionDescription localPartitionInfo : candidates) {
            ...
            MysqlDateTime expirationDate =
                MySQLTimeCalculator.addInterval(partitionDate, definitionInfo.getIntervalType(), expireInterval);
