 ...
 */
public class BitAnd extends AbstractAggregator {
    private NullableObjectGroupState groupState;

    public BitAnd(int index, DataType inputTypes, DataType outType, int filterArg) {
    ...
    public void appendInitValue() {
        groupState.append(returnType.getMaxValue());
    }

    @Override
    public void resetToInitValue(int groupId) {
        groupState.set(groupId, returnType.getMaxValue());
    }
    ...
    public void accumulate(int groupId, Chunk chunk, int position) {
        ...
        groupState.set(groupId, returnType.getCalculator().bitAnd(value, beforeValue));
    }
